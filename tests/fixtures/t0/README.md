...
| `label_sides_h` | ONE resistor placed **horizontally**, pins at ∓0.3u, one long net name on each — with tscircuit's REAL label shape: the plate `center` on the opposite side of `anchor_position` from what `anchor_side` names | **label plates fired INWARD across the body.** The converter passed `anchor_side` straight into its (angle, justify) table, but `anchor_side` names the label EDGE the anchor sits on — the REVERSE of the reach. MEASURED over all 7 live boards: 1504/1504 labels have `anchor_side` exactly opposite `center - anchor_position`, 0 missing `center`. Both plates must end CLEAR of the body, and the net names are deliberately longer than the pin span so no placement could ever separate two inward plates (canon M-DISC) |
| `label_sides_v` | the same part placed **vertically** (rotation 90), pins at ±0.3u | the orientation `label_sides_h` cannot discriminate, and the home of the SECOND defect: `LABEL_ANG_JUST['bottom']` was `(270, 'left')`, which KiCad renders reaching **UP** — measured from rendered SVG ink, since a global_label's angle selects only the AXIS and `justify` alone selects the sense. Pre-fix the two defects CANCELLED on `anchor_side: bottom` labels and did not on `anchor_side: top` ones, so repairing either alone would have broken the ~155 fleet labels the pair was getting right |
| `rotated_placement` | R1..R4 declared as real `pcb_component` elements at tscircuit rotations **0 / 90 / 180 / 270** | **canon M-DISC — the all-zeros fixture set.** The other five fixtures carry ten `"rotation"` values and ALL TEN ARE 0, and none of them declares a `pcb_component` at all, so the PLACEMENT bridge (`circuit_json_to_kicad_pcb.py`) had no fixture whatsoever. Its operator is `korient = (-tsc_rotation) % 360` — a NEGATION, the exact shape of the `jlc_twin.xform()` handedness bug that had FIVE copies and cost 22 wrong CPL rotations. `formB(a) == formA(-a)` identically and both coincide at 0/180, so a 0/180-only fixture set is satisfied by BOTH candidate mappings and proves nothing about a sign. This fixture makes the set discriminating; `t1_rotation_fixtures.py` enforces that it stays so |
