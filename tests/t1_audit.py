...
@test("A-BODY grades the CPL counter without hiding missing optional manual "
      "bodies", kind="known_bad")
def t_abody_separates_cpl_from_manual_body_coverage():
    """A twin can deliberately include hand-solder bodies beyond the CPL.
    A-BODY promises JLC placement coverage, so it must grade 2/2 here while
    the same generated report honestly retains a missing J1 manual body."""
    d = tmpdir("abodypop_")
    _multi_board_project(d, ["boarda"], ["boarda-v1.0-2026-01-01"])
    mm = (d / "07_releases" / "boarda-v1.0-2026-01-01" /
          "verification" / "missing_models.txt")
    mm.write_text(
        "# GENERATED by jlc_twin.py NO-BODY pass — do not edit.\n"
        "# source of the population set: 2 CPL placements + 1 manual body\n"
        "# bodies mounted: 2/3\n"
        "# CPL bodies mounted: 2/2\n"
        "# manual bodies mounted: 0/1\n"
        "J1\t\tfootprint carries no 3D model entry\n")
    run([KPY, POLICY, d, "--skip-drc", "--board", "boarda"])
    md = (d / "06_build" / "policy_audit.md").read_text()
    row = [l for l in md.splitlines() if "| A-BODY |" in l]
    check(row and "| A-BODY | PASS |" in row[0],
          f"A-BODY confused optional manual coverage with the CPL: {row}")
    contains(row[0], "CPL bodies mounted 2/2",
             "A-BODY did not state the population it graded")
