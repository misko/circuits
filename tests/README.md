...
| `t1_pipeline_xtrace.py`, `t1_pipeline_canary_usb.py`, `t1_pipeline_canary_pluto.py` | a dedicated Bash xtrace channel maps the exact USB Hub 3S v4 and Pluto RX2 8-way reuse drivers onto hash-pinned stage catalogs while retaining unmapped executable evidence | generic shell logs, stale line maps, truncated traces, setup/failure-handler masquerade and catalog/driver drift are refused |
