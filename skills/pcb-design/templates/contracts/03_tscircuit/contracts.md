...
| Class | Paths | Rule |
|---|---|---|
| **HAND-EDITED truth** | `src/<board>.tsx`, `net_aliases.txt`, `manifest.yaml`, `parity_padmap.txt`, `sealed_ref.txt`, `package.json`, `README.md`, `GENERATE.md`, `contracts.md` | the only files a human writes here; `package.json` pins an exact producer version, never `*` |
