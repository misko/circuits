...
| `power_stages.yaml` | E-SWDRV switching-stage compatibility: controller minimum gate-drive capability, bias, frequency, MOSFET population, and maximum/qualified-maximum gate charge. Empty boards explicitly state `no_external_gate_drive_stages`. Graded before layout. |
...
| `stages[].*` | `early_design_check.py` | controller minimum current, bias, frequency, MOSFET count, gate charge, and thermal model |
