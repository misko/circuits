...
| `length_match.<G>.phase.cross_section` | OWED | the named transmission-line cross-section is documentation only; `copper_length_audit.py` derives delay from stackup inputs and never validates this declaration, so disagreement is silent |
