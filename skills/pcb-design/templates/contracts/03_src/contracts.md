...
| `placement_gates.json` | OPTIONAL config/waivers for the SHARED `placement_gates.py` gate (proposed canon IDs P-OUT pads-inside-outline, P-CAP corridor crossing-demand vs capacity — the two checks the cooksense routing D-BACK of 2026-07-23 proved missing). Missing file = defaults. Waivers (`waive`, `out_ok`, `cap.waive_cuts`) live HERE, evidence (`why`) required | SHARED `placement_gates.py` |
