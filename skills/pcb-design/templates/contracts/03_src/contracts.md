...
| `thermal_vias` | Optional fabrication-via declarations. Preferred `fields[]` rows name `ref` or `refs`, an owning `pad`, `size`, `drill`, and non-empty footprint-relative `at` coordinates; the generator resolves the pad's live nonzero net and emits true board-level vias without modifying the library-linked footprint. Legacy `promote_heatsink_pads[]` instead replaces every drilled `pad_prop_heatsink` subpad on each named footprint with an identical true via and clears the now-modified embedded footprint's library ID while preserving its source FPID in the description. These are the JLC fill/cap boundary: component PTH holes are not vias and must not silently inherit a via-covering order. Unknown refs/pads, empty matches, invalid geometry, slots and unnetted marks refuse. | SHARED `generate_board_generic.py` |
| `route.yaml` | routing + stitch config plus `flow.pre_route_reviews` exact-artifact bindings and `route.preflight_critical_pairs` (R-PAIRMAP/R-CRITESC): every critical differential pair is assigned to the differential engine, length-match group, allowed layers, and via policy before routing; the realized board is rechecked after stitch. | SHARED `pre_route_review_check.py`, `critical_route_check.py`, `route_and_stitch_generic.py` |
...
| 1b | CHEAP SEMANTIC BATTERY immediately after netlist export: `$S/early_design_check.py .` first (D-SPEC/E-PATH/E-SWDRV/E-SURGE), then net-label survival, electrical invariants/ADR coverage, power topology/margin/off-control, count parity, and circuit-vs-parts BOM source. Upstream red stops the pipeline before schematic review or layout. | any python3 |
...
| 2a | `$S/pre_route_review_check.py . --phase schematic` — **PR-REVIEW**: before placement/routing spend, an independent topology review must say `SOUND` and bind the exact exported-netlist SHA, aggregate `02_parts` SHA, and—on adopted projects—the aggregate design-rule SHA (`03_src/rules/*.yaml` + `route.yaml`). Requirements/ratings edits stale the witness. | any python3 |
...
| 4d | `$S/pre_route_review_check.py . --phase placement --board 04_kicad/<board>.kicad_pcb` — **PR-REVIEW**: independent pin/layout/render reviews plus same-camera A-RENDER bind the exact board and adopted design-rule SHA before routing. | any python3 |
