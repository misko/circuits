...
| 1r | THE HUMAN SCHEMATIC, DELETED THEN REGENERATED: `rm -f 03_tscircuit/build/schematic.svg 03_tscircuit/build/schematic.pdf`, then `tsci export -f schematic-svg` + `rsvg-convert -f pdf`. The `rm -f` is the load-bearing line. `07_releases/contracts.md` ships this PDF as `pdf/schematic.pdf` and `tsci build` never writes it, so before this stage it was whatever the last `gen_tscircuit.sh` run left — MEASURED on pluto-rx2-8way-v2 2026-07-30 at **14:47:14 beside an 18:42:05 `circuit.json`**, i.e. a release could ship a schematic that does not match its own netlist with every gate green. Rendering into a path that still holds the previous revision cannot fail safely, so the two producers carry `|| true` ON PURPOSE: the failure mode becomes ABSENCE and the GATE at [1a] names it (`F-RENDER`), instead of `set -e` aborting with no finding or a stale PDF surviving | bun/tsci + `rsvg-convert` |
...
| 4 | `03_src/audit_board.py` (placement/pad invariants) **IF IT EXISTS — the call is GUARDED, and its absence is ANNOUNCED, never silent.** A ZERO-BESPOKE-PYTHON board (ADR-0002, the go-forward default) has no such script: placement comes from `floorplan.yaml` and every invariant it would have hand-checked is a SHARED gate running beside this step. Calling it unconditionally aborts every such board at `set -e` (the template did, measured on pluto-rx2-8way-v2 2026-07-30); skipping it with a bare `if [ -f ]` is the WORSE repair, because a board that LOST its audit script then reads as one that passed a gate that never ran (canon M-COVER, arriving in a driver instead of a checker). So: `if [ -f … ]; then …; else echo "no 03_src/audit_board.py (generic-backend board)"; fi`, pinned by `tests/t1_rebuild_templates.py` (`audit_board_guard_ok`) in BOTH directions. Then `$S/placement_gates.py 04_kicad/<board>.kicad_pcb --config 03_src/placement_gates.json` (SHARED, canon IDs: P-OUT pads inside the real Edge.Cuts polygon; P-CAP static corridor crossing-demand vs capacity — both post-placement/pre-routing, born of the cooksense 13h routing D-BACK), which is NOT optional | `/usr/bin/python3` |
