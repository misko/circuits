...
| S9 | Spec tensions surfaced at COMMISSION (D-SPEC): every numeric requirement tested against the governing standard and the sourceable-part envelope; each tension gets an ADR + a flagged BRIEF row — never silently built out-of-spec, never silently downgraded | [H] BRIEF `Spec tensions` table filled ("none found" only after checking) + ADR per tension | "USB-C 6A" exceeds Type-C 3A/PD 5A; "USB-A 2.5A" exceeds every stocked receptacle rating — the 2026-07-21 clean-room agent caught both by instinct (ADR-0005); instinct is not a gate |
...
| P7 | Escape feasibility + fab tier decided at PART SELECTION: every multi-pin part.yaml carries an `escape:` block (style, pitch, tier_required — computed by `escape_check.py` against `references/fab_tiers.yaml`), and no used part requires a tier above the `fab_tier:` declared in `03_src/rules/nets.yaml`. Raising the tier requires the D-TIER ADR + the tier's ORDER_README line | [M] P-ESC (block present + agrees with live recomputation; declared style/pitch cross-checked against footprint text) + [M] P-TIER (tier_required rank <= declared fab_tier rank) | clean-room 3S stall 2026-07-20: SY8368 QFN-10 @ 0.5mm pitch selected with fab tier defaulted to standard; the unmade ADVANCED decision surfaced 2 stages later as `drill_out_of_range` at DRC. The analysis lived only in a chat report and evaporated |
