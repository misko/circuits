#!/usr/bin/env python3
"""Generate 04_kicad/lipo3s_usb_hub.kicad_pcb from the netlist + floorplan.
...
NETLIST = HERE.parent / "06_build" / "netlists" / "lipo3s_usb_hub.net"
PCB = K / "lipo3s_usb_hub.kicad_pcb"
STD = "/usr/share/kicad/footprints"
PROJ = str(HERE / "lib" / "lipo3s_usb_hub.pretty")
...
def write_lib_table(libs):
    rows = ['(fp_lib_table', '  (version 7)',
            '  (lib (name "lipo3s_usb_hub")(type "KiCad")(uri "${KIPRJMOD}/../03_src/lib/lipo3s_usb_hub.pretty")(options "")(descr "project footprints"))']
    for lib in sorted(libs - {"lipo3s_usb_hub"}):
...
def load_fp(fpid):
    lib, name = fpid.split(":")
    root = PROJ if lib == "lipo3s_usb_hub" else f"{STD}/{lib}.pretty"
...
fp.SetFPID(pcbnew.LIB_ID("lipo3s_usb_hub", "{name}"))
