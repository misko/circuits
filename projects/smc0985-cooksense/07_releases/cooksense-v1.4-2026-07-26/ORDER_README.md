...
> ## RELEASE v1.4 — DOCUMENTATION-ONLY SUPERSEDE OF v1.3
>
> **The BOARD is unchanged and is still revision v1.3** — that is what the
> silkscreen reads, and it is correct. `fab/`, `source/`, `3d/` and `pdf/` are
> **byte-identical** to v1.3 (verified by directory-wide sha256). 1047 vias,
> 3925 tracks, DRC 0/0/0. **H4 genuinely passes at 6.5984 mm creepage.**
>
> **This release exists for one reason:** v1.3's MANIFEST `READ BEFORE ASSEMBLY`
> block named the notch's **NEAR** bank where the **FAR** bank governs, and
> printed the straight-line clearance 3.8286 mm under a creepage claim. §1 of
> this document had already been corrected; **its summary in the MANIFEST had
> not.** An assembler following the v1.3 MANIFEST would have measured 2.200 mm
> to the nearest edge, applied the block's own rule, and concluded the
> **specified** DIN 125 A2.7 washer already bridges the notch — condemning the
> part the clause exists to protect.
>
> **If you hold v1.3: its gerbers are correct and orderable. Do not use its
> MANIFEST assembly warning — use this one.**
