# ORDER README — cooksense MAIN board **v1.3 silk / v1.5 release** (project smc0985-cooksense)
...
> and the barrier drops from **6.3811 mm to 4.7195 mm** — a FAIL — with nothing
> visible to show it.
...
| 6.2 mm | | 6.4835 mm | 3.9286 mm | PASS |
| 6.3 mm | | 6.4265 mm | 3.8786 mm | PASS — hard limit |
| 6.38 mm | | 6.3811 mm | 3.8386 mm | PASS — last passing value |
...
| 6.93 mm | 6 mm A/F hex standoff, across corners | 3.7057 mm | 3.5636 mm | FAIL |
| 8.0 mm | DIN 9021 body washer | 3.0286 mm | 3.0286 mm | FAIL |

**The cliff is a genuine discontinuity, not a steep slope:** 6.38 mm → 6.3811 mm
...
**2026-07-27**, taken against THIS release's BOM (not v1.4's; finding a stock-out
after the seal costs another release, which is what v1.4 -> v1.5 just paid for).

**A STOCK PASS IS NECESSARY AND NOT SUFFICIENT, AND THE FLEET HAS THE RECEIPT.**
`stockCount` is LCSC **catalog** stock; JLC's assembly uploader allocates feeders
from a **different pool**. usb-hub-3s-v3 v1.10 sealed with
`{"lcsc": "C25744", "status": "OK", "stock": 291}` in its own evidence and JLC
refused that line hours later. A FAIL from this gate is real; a PASS is a
precondition, not a promise. Watch `minPurchaseNum` as well as `stockCount` —
that is what caught C25862 here, whose 7463-piece minimum sat against
double-digit stock while `stock >= 5 x qty` passed it.

Full scan: **54 coded lines, 53 clear at 5 boards, 1 problem** (and 2 uncoded
lines, hand-solder, not graded by this tool). The thinnest lines:

| Ref | LCSC | stock (2026-07-27) | need @5 boards | note |
|---|---|---|---|---|
| **J_ISOLOOP (KF350-3.5-4P)** | **C42400616** | **0** | 5 | **the only failing line, and it is unplaced by design** — self-supplied, hand-soldered, off the CPL. See §4 and the note below |
| U_ADC (MCP3208) | C16939 | **124** | 5 | thin, and thinner than v1.4's 128 |
| U_EFUSE (TPS259573) | C2653844 | 160 | 5 | thin |
| J_PWR Micro-Fit | C587657 | **172** | 5 | thin, and down from v1.4's 778 |
| F1 polyfuse | C89650 | 244 | 5 | thin |
| U_EXP (MCP23017-E/SS) | C506653 | 896 | 5 | |
| U_ULNA/B (ULN2803ADWR) | C9683 | 2315 | 10 | |
| U_ONESHOT (CD74HC221M96) | C133954 | 2531 | 5 | |
| U_AND*/U_CAND*/... (SN74LVC1G11) | C22046 | 3706 | **50** | 10 refs/board |
| **R_BID0..R_TEMPOK (10 kΩ) — NEW** | **C60490** | 8 404 363 | **85** | **NOT C25744, which is stockCount 0. Extended part: feeder fee applies** |
| **R_ILM (1.2 kΩ) — NEW** | **C138040** | 472 208 | 5 | **NOT C25862, whose minPurchaseNum is 7463** |
| **R_OPENT (62 kΩ)** | **C37825** | 127 383 | 5 | **NOT C25915 — see §12** |
| **R_WDPETPD (1 kΩ)** | **C11702** | 1 178 285 | 45 | **NOT C25741 — see §12** |

A-STOCK grades every coded, PLACED line at stock >= qty x build_quantity (5).

**C42400616 (J_ISOLOOP) IS STILL 0 AND WAS DELIBERATELY NOT SUBSTITUTED.** JLC
stocks no KF350 4-pole line at all (all three 4P codes read 0) and has no CAD for
any of them — the control query the same minute returns C474892, the 2-pole part
of the same family, at live stock, so the zeros are the library's answer and not
a dead field. The part is THT on an SMT-only order, so JLC would not place it
even if it were stocked: it is `not_assembled`, `exclude_from_pos_files`, and off
the CPL. **The options, stated rather than forced:**

- **(a) WHAT THIS RELEASE DOES — buy it outside LCSC.** KF350-3.5-4P is a
  commodity 3.5 mm 4-pole screw block, widely stocked by other distributors. Any
  350-family part with a **14.0 × 7.0 mm body and 4 poles at 3.5 mm pitch** drops
  into the existing land (XY350-3.5-4P, DG350-3.5-4P). **No board change, no
  review invalidated.**
- **(b) A DIFFERENT BLOCK — and it would VOID this board's review.** JLC does
  stock 3.5 mm 4-pole alternatives (KF2EDGV-3.5-4P C441323 at 9954,
  KF2EDGR-3.5-4P C441173 at 9999, XY350V-3.5-4P C557656 at 5004), but they are
  **pluggable 2EDG-family header+plug pairs with a different body and a different
  pin projection** — a footprint change, which moves copper, which re-opens the
  ISO_CONTACTOR 2.0 mm moat, the H4 courtyard solve that closes at 0.09–0.35 mm,
  and every render/polarity verdict this board carries. **Not done, and not
  recommended for five prototypes.**
- **(c) Declare it `not_assembled` and leave the pads bare.** Already true of the
  machine order. The four poles are the isolated E-stop / contactor loop, so
  the board is not functionally complete without it — §11's pole legend is the
  field-wiring gate either way.
