...
## v1.4 — DOCUMENTATION-ONLY SUPERSEDE. THE CORRECTION DID NOT REACH THE SUMMARY.

**The defect is not "a sentence was wrong". It is "a correction landed in one
document and not in its summary, and nothing checked the copies."**

The tenth (scoped) lens caught that §1's washer clause named the notch's NEAR
bank where the FAR bank governs. **I fixed `ORDER_README.md` and sealed v1.3 with
the MANIFEST's `READ BEFORE ASSEMBLY` block still carrying both original
errors** — in the block I had myself called "the fast path a reader stops at",
in the same report where I noted the withdrawn 30 V attribution had survived in
five places *including the MANIFEST*. I diagnosed the propagation pattern and
shipped an instance of it in the same release.

| defect | v1.3 MANIFEST | correct |
|---|---|---|
| governing bank | "3.200 mm from the notch's **near** edge" | near (south) bank **2.200 mm**; far (north) bank **3.200 mm**; **the FAR bank governs** — bridging requires reaching the far side |
| collapse figure | "creepage collapses 6.5984 → **3.8286**" | 3.8286 is the straight-line **CLEARANCE**. True post-collapse **creepage 4.7195 mm**; the cliff is **6.3815 → 4.7195** |

**Reader harm:** measure centre-to-nearest-edge, get 2.200, read 3.200, apply the
block's own rule (radius ≥ that distance ⇒ bridges) — and conclude the
**specified** DIN 125 A2.7 (r 3.000) already bridges. The clause condemns the part
it protects.

### The acceptance test is now a SWEEP, not a re-read

Because the failure class is *copies*, the gate is: grep **every shipped file**
for the retracted strings and report counts before and after.

| pattern | v1.3 total | v1.4 total | **v1.4 USES (i.e. real defects)** | what the rest are |
|---|---|---|---|---|
| `near edge` | 1 | 1 | **0** | the single v1.4 hit is the sweep table row below, quoting the pattern it greps for |
| `3.8286` | 6 | 10 | **0** | 4 labelled as *clearance* beside the creepage column, 4 in retraction/defect-explanation text, 2 in this ledger's own tables |
| `30 V working, PD3, material group IIIa` | 5 | 6 | **0** | 5 inside the paragraph that withdraws it and names `opto_isolation_2mm` as its true owner, 1 in the sweep row |

**Counting method, stated because the naive count is misleading:** the totals RISE
from v1.3 to v1.4 even though the defects were fixed, because **documenting a
retraction quotes the retracted string.** The column that matters is
**USES** — instances asserting the wrong thing — and it is **0** in v1.4 against
**2** in v1.3 (`MANIFEST.txt:4` "near edge", `MANIFEST.txt:6` the unlabelled
3.8286). A sweep gate for this class must classify hits, not count them; a pure
count would have read this fix as a regression.

**And the sweep caught one in this very ledger.** The first version of this table
claimed "all labelled" for `3.8286`. It was false: L9-1's row above printed
`6.400 → 3.8286 FAIL` inside a **creepage** list, unlabelled — retracted 24 lines
later by L10-4, but wrong where it stood. Now labelled. **The gate found a defect
in the report of the gate.**

### Second finding, as asked: `git_sha` / `git_dirty` were genuinely absent

M-REL grades exactly those two lines and neither existed in any MANIFEST this
board has shipped. v1.4 adds them (`git_sha: ac47fc75…`, `git_dirty: no`). **A
gate that grades a field no artifact emits cannot fail** — same family as A-EVID
and the `row_kind` blind spot, now the fourth instance recorded on this board.

### Third finding, found while running the gate

`release_freshness_check.py` returned **FAIL with 22 findings** on v1.4, every
one of the form *"X is sha256-IDENTICAL to v1.3's X — a changed board must not
ship an earlier release's generated output"*. **For a documentation-only
supersede that identity is the REQUIREMENT, not a defect** — the gate's verdict is
inverted for this release class. The tool already has the right mode
(`--docs-only-supersede PRIOR_DIR`), which asserts fab/, source/ and 3d/ are
byte-identical and passes: **FRESHNESS: PASS**. Recorded because invoking the
default mode on a docs-only release produces a confident, wrong FAIL.

### Payload identity — the hard requirement, verified

`fab/` (19 files), `source/` (11), `3d/` (2) and `pdf/` all **byte-identical** to
v1.3 by directory-wide sha256, and asserted independently by the freshness gate.
80 digests self-verified, 0 mismatches. **The board did not move.**

---

# v1.6 — THE THREE OPEN P1s FROM THE 2026-07-27 ADVERSARIAL AUDIT

Source review: `verification/redteam_adversarial.md` (verbatim copy of
`08_reviews/2026-07-27_v1.5_redteam_adversarial.md`, report-only, committed
f8427c5). It raised 0 new P0 and 7 P1. **A12 was closed without a reseal**
(539ecf0 — the S-VER waiver's fabricated citation). A1, A2 and A3 are closed
here. **All three are paperwork; the copper does not move**, and all three
HARDWARE fixes are USER DECISIONS deferred to the next electrical revision.

Every one was RE-VERIFIED against this archive's own `source/cooksense.net` by
an independent s-expression set query (canon M1) before acceptance. The full
measurement is `verification/crossplug_and_permission_defaults.md`; the
disposition ledger with the same rows is `08_reviews/DISPOSITIONS.md`.

| id | finding | verdict of the re-verification | disposition |
|---|---|---|---|
| A1 | An SHT45 pod harness in `J_MODE` energises the coil rail with the whole AND chain and the Manual rail-cut bypassed; §10 modelled 3 of 5 identical housings | **CONFIRMED, every number.** `COIL_EN` = 3 nodes, sole hold `R_COILENPD` 100 kΩ, no ESD, no series R. Pod pull-up 10 kΩ → 3.000 V on the 2N7002 gate (spec `V_GS(th)` max 2.5 V); 4.7 kΩ → 3.152 V. `J_MODE` is 38.29 mm from `J_RH_EXHAUST`. **Refinement:** the audit's `R ≤ 175 kΩ` bound assumes a min-threshold subthreshold device and is the softer of the two results — labelled as such. **New root cause:** the 2026-07-23 re-pinning modelled a cross-plug as a passive BRIDGE, which is the wrong model for a harness that SOURCES current | ORDER_README §10 rewritten — five housings, the fail-safe claim **WITHDRAWN**, a 20-cell matrix, a mandatory discipline (§10.5), copper fixes named and deferred (§10.6). Declared gap 19. `R_COILENPD` = 100k now machine-checked (E-INV `part_value`, RED-verified) |
| A2 | Four permissions have no pull; the tsx's "the other twelve are pulled restrictive" is falsified | **CONFIRMED and EXTENDED — and the audit's characterisation CORRECTED.** 11 of the 18 chain nets carry no pull, not 4. But the twelve the tsx counts are exactly BRIEF D10 item 8's Pi/expander authorization lines and **all twelve genuinely ARE pulled restrictive**: the claim is wrong in SCOPE, not arithmetic. No single part floats all four — `U_SCHM` gives three, `U_WD` the fourth. **New finding:** MCP23017 `GPPUB` (DS20001952C §3.5.7, 100 kΩ) can make the float deterministically PERMISSIVE with one register write | ORDER_README §7a-2 makes `GPPUB = 0x00` a REQUIRED firmware invariant; declared gap 20 carries the 11-of-18 measurement, both single-part cases and the scope correction. Four (eleven) 0402 pull-downs are the hardware fix — copper, deferred |
| A3 | `REARM_N` held low forces the NAND latch's forbidden state | **CONFIRMED, and the persistence is worse than stated.** One driver (`U_EXP.26`), no button/pin/TP. Held low the latch degenerates to a combinational repeater and loses its memory. Power-up-forced-SET confirmed via TPS3823 t_d and MCP23017 `IODIR` POR `1111 1111`. **New:** `EXP_RST_N` has NO DRIVER, so the defeat survives every Pi reboot though not a 3V3 cycle | ORDER_README §7a-1: the driver invariant, the latch analysis and a REQUIRED negative bring-up test. §7 step 3 says PULSE. Declared gap 21. `R_REARMPU` = 100k now machine-checked (RED-verified). Hardware fix (edge-detect on `REARM_N`) deferred |
| — | Bonus, from the re-verification | `SN74HC14DR/part.yaml` claims 3A/4A/5A/6A are unused — all six gates ARE used. `cooksense.tsx:632` contradicts `:637-638` on which `J_MODE` pole is which; the netlist says 632 is stale | Declared gaps 22 and 23. ORDER_README §10.1's table is named as the harness authority in place of the source comment |

### Payload identity — v1.6

`fab/`, `source/` and `3d/` byte-identical to `cooksense-v1.5-2026-07-27`,
ASSERTED (not waived) by `release_freshness_check.py --docs-only-supersede`,
which sha256s every file in both directions and FAILs on any addition, removal
or difference. `pdf/` likewise identical. **The board did not move, and this
release never opened it.**
