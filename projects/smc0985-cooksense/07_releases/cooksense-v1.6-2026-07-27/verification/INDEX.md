...
cooksense **v1.6, 2026-07-27**. The board silkscreen still reads `v1.3` and that
is correct — `source/cooksense.kicad_pcb` is md5-identical to v1.5's, v1.4's,
v1.3's and `04_kicad/`'s (`420445b5141dd1111eccab038c68511b`).

**v1.6 IS A DOCUMENTATION-ONLY SUPERSEDE AND REGENERATES ALMOST NOTHING.**
`fab/`, `source/` and `3d/` are byte-for-byte v1.5's, asserted by
`release_freshness_check.py --docs-only-supersede`. **Six files below moved; the
rest are carried and, because the board did not move, are still current.** The
"currency" column now reads v1.5 for every carried file — that is the honest
label: those files were produced for v1.5 and describe the same board.
...
|---|---|---|
| `INDEX.md` | this file | **v1.6** |
| **NEW OR REGENERATED FOR v1.6 — six files** | | |
| `crossplug_and_permission_defaults.md` | **the v1.6 evidence.** The three open v1.5 P1s re-verified from `source/cooksense.net` by an independent s-expression set query (canon M1): the 20-cell cross-plug matrix and its numbers, the 11-of-18 permission-default measurement, the `REARM_N` latch analysis, and the two places the re-verification DISAGREED with the audit | **v1.6** |
| `redteam_adversarial.md` | verbatim copy of `08_reviews/2026-07-27_v1.5_redteam_adversarial.md` — the report-only audit this release closes. 0 P0, 7 P1 | **v1.6** (review dated 2026-07-27) |
| `dispositions.md` | v1.3's ledger **plus a v1.6 section** dispositioning A1/A2/A3 with the verdict of each re-verification | **v1.6** |
| `einv.txt` | E-INV **85/85** against `source/cooksense.net` — was 83/83; v1.6 adds `part_value` asserts pinning `R_COILENPD` = 100k and `R_REARMPU` = 100k, the two numbers ORDER_README §10.2 and §7a-1 compute their bounds from. Both RED-VERIFIED (substituting 10k gives `E-INV FAIL: 2/85`) | **v1.6** |
| `policy_audit.md` | regraded against **this** release directory | **v1.6** |
| `freshness_exceptions.txt` | **deliberately lists ZERO paths** — docs-only mode ASSERTS the fab/source/3d identity instead of waiving it file by file, which the 07_releases contract requires. The file explains why the other four narrow modes do not describe this release | **v1.6** |
| **CARRIED FROM v1.5 — the BOM, the board and the netlist are all byte-identical, so these are still current** | | |
...
| `einv_red_verification.md` | proof the E-INV checker CAN fail, against the 83 invariants v1.3 shipped. **v1.6's two NEW asserts have their own RED proof**, recorded in `crossplug_and_permission_defaults.md` §4 | v1.3, current; v1.6 addendum in the crossplug file |
...
| `redteam_topology.md` — **AND READ THIS LINE** | its line 58 says "all 12 others are pulled restrictive (REARM_N correctly pulled up)". That is the SAME claim v1.6 corrects. It is **not edited** — a dated review is a record of what a reviewer said, not a wiki — and the correction lives in `crossplug_and_permission_defaults.md` §2.3 and ORDER_README §13 gap 20 | review v1.3, **one claim superseded** |
