    NOTE: this release is a FIX-PASS (canon "Verification scoping"), so its
    red-team breadth is ONE INTEGRATED fresh-context lens covering BOTH
    required lenses in a single pass, not two separate reviews. This file and
    redteam_layout.md / redteam_topology.md are therefore the SAME review,
    archived under both required names. THIS FILE carries lens (b) layout / mechanical / fab-data:
    verdict ORDER (the P1 it named is CLOSED — see verification/dispositions.md R1).
    Verbatim archive: 08_reviews/2026-07-27_interposer-v1.1_redteam_integrated.md
