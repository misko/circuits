...
| T2 | mate the OEM CN1 | receptacle is a LATCHED variant, brief's TRIO-MATE candidate is friction-only (mismatch found from photos); exact OEM MPN unidentified | replicate tail geometry incl. the two lock-slots; coupon G1/G2 mandatory before any Board C fab; TRIO-MATE candidate for our side only | 0005 | yes (photos) |
