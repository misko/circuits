...
$PY "$SKILLS"/import_krt.py 06_build/route/r3.kicad_pcb \
    04_kicad/crow_mic_pod.kicad_pcb 04_kicad/crow_mic_pod.kicad_pcb 2>/dev/null | grep imported
...
kicad-cli pcb drc --severity-all --refill-zones --schematic-parity --format json \
    -o 06_build/drc/gate.json 04_kicad/crow_mic_pod.kicad_pcb >/dev/null
python3 - <<'PYEOF'
import json
