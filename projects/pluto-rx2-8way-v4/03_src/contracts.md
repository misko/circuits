...
| `pipeline_shadow_*.json` | NON-AUTHORITATIVE, exact-driver-hash-bound catalogs of the full and deterministic-reuse legacy stage order, applicability, accepted outputs, and deferred/postcheck authority. They are inert ADR-0008 equivalence evidence: changing a driver invalidates its catalog; the catalogs do not execute or replace either driver | SHARED `pipeline_catalog.py`, `pipeline_shadow.py`, `pipeline_xtrace.py`; `tests/t1_pipeline_canary_pluto_v4.py` |
