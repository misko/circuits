...
## 2026-08-18 14:12 — v2 USB-only checkpoint

- entered routing only after the exact two-USB-C connector subject passed the
  machine gate and received explicit user approval; no PD or high-current
  copper was allowed into this stage;
- the TYPE-C-31-M-12 contact field required a bounded 0.15 mm connector-launch
  clearance and the exact drawing's 0.2194 mm locating-hole-to-GND-land space
  required replacing the inherited 0.25 mm hole preference with a 0.20 mm
  advanced-tier floor. Fresh r0 DRC then had zero clearance/hole findings;
- a first stochastic downstream run found 3/4 pairs and stopped on a P1/P2
  rip-up cycle. Original port order found 4/4, but physical DRC rejected its
  short, clearance and uncoupled-length findings. Neither partial candidate
  was promoted;
- because all downstream endpoints and placements are byte-for-byte unchanged,
  the established v1 source geometry was replayed onto the exact v2 r0. The
  new USB-C upstream path was authored separately with a two-via physical
  crossover; D+/D- net identities never swap;
- exact checkpoint: r0 SHA256 `62df03151bba81f11b716a387a121473b0bace1c1b4686d569f9b53c8ca52dcc`,
  USB prefix SHA256 `2fa29256c1b8812a92dc9952f773b43f42832c3d2cc4296b018e810139abf596`;
  P-ROUTEBASE passes 169 footprints, eight base vias and 185 prepared
  segments; physical DRC has zero hard findings; 10/10 critical pairs are
  connected; immutable candidate grading ACCEPTED the prefix through
  `usb_upstream` at r2. No FINAL marker exists.

Reflection: duplicate-contact USB-C breakouts need a polarity-order sketch and
an explicit physical-crossover decision before invoking a coupled router.
Unchanged, already-reviewed copper should be reused only through an exact-base,
hash-bound checkpoint—not rediscovered stochastically and not copied by visual
similarity. Most time in this stage was spent proving provenance and physical
DRC after a fast fail-closed router result; the bounded wave prevented any PD
or power-routing backtrack.

## 2026-08-18 14:29 — v2 PD and high-current power checkpoint

- regenerated the exact placement after making the TPS56637 bootstrap
  capacitor loop-adjacent; the placement gate rejected the first attempted
  pose because its anchored courtyard overlapped the buck, and the corrected
  pose passes 162/162 placement checks with zero pad/courtyard collisions;
- authored deterministic PD-switch, bootstrap, feedback, aggregate-eFuse and
  protected-5V distribution copper in `build_power_prefix.py`. The exact
  prefix has no hard copper DRC finding and no open item on `VBUS_PD_RAW`,
  `VBUS_PD`, `PD_SW`, `PD_BOOT`, `P5V_REG` or `P5V_PROTECTED`;
- split the adjacent P5V_REG/P5V_PROTECTED source zones at the eFuse boundary.
  Equal-priority overlapping zones had produced competing fill and a false
  visual impression of continuity; the corrected ownership boundary fills
  each rail only on its own side of the series device;
- exact hashes: prepared r0
  `fef090e6fb4ed1a24cfda8d7f593df423aa1957d99858a4d1f136af88d4183d0`;
  authenticated power prefix
  `cbae944abeb4b3979114ea0c165aa368b4dce02aad2cae5eafa778ca06afa41e`.
  P-ROUTEBASE passes 169 footprints, eight base vias and 185 prepared
  segments; immutable candidate grading ACCEPTED the prefix through the new
  `power_input` wave at r3; 10/10 USB pair contracts still pass;
- A-VIA grades the exact eight-hole protected-5V series bank at 4.4 A
  continuous capacity for a 10 C rise basis versus the 3.0 A continuous trunk
  contract, with zero credit for via fill;
- the geometry-bound 35 um copper skeleton gives 75 C predicted drops of
  28.23 mV to the left port-1 branch, 48.74 mV to the worst modeled right
  branch and 22.92 mV to management. This is copper-only evidence: via barrels,
  lands/joints, switches, zone spreading and connector contacts remain
  first-article four-wire/thermal obligations;
- power-topology audit passes 7/7 rails and both converters. Its conservative
  input calculation is 2.1 A at 13.5 V against the 3 A PD-side contract.
- the current checkpoint is intentionally owned by `route_usb_v2.yaml`.
  `03_src/route.yaml` is still the inherited v1 final-route contract, so the
  canonical full-route acceptance compositor is not yet authoritative for v2;
  consolidating the accepted USB/power prefix and the remaining control waves
  into that single contract is the first task of the next routing stage.

Reflection: switching cells need explicit pre-route adjacency contracts for
the bootstrap loop and zone ownership on both sides of every series device.
The productive order was local switching loop first, then feedback/sense,
then the high-current layer transfer and distributed trunk. Quantitative via
and copper checks belong before control routing, while the geometry is still
cheap to change. The remaining 11 starved-thermal and six silk-over-copper
items are deferred partial-board findings, not accepted final-board waivers.

## 2026-08-18 15:10 — v2 control and low-speed signal checkpoint

- replayed reviewed v1 control copper only for nets whose complete endpoint
  pad signatures matched the exact v2 placement. Sixty-seven nets qualified;
  changed USB-C, VBUS-sense, port-1 enable and power-cell nets were excluded;
- corrected placement reachability before routing around it: R_CC1 remains
  above the upstream D+/D- corridor, while R_PWR_EN1 and the VBUS detector
  divider now sit in reachable cells beside U_PWR1 and J_DATA. Regeneration
  passes 162/162 anchors, 45 assertions, zero pad collisions and zero anchored
  courtyard collisions;
- added deterministic local routes for both USB-C CC sets, upstream VBUS
  detection, PWR_EN1, crystal branches, both regulator control networks and
  the aggregate-eFuse timing controls. The TPS56637 feedback network and
  AP63203 bootstrap pad order were corrected before accepting copper;
- exact signal-prefix SHA256 is
  `5287ee243b416a17bbe5023de4aa79858b956bb4f67f23246662a915495468ec`.
  With matching rule sidecars, native DRC reports zero non-thermal hard
  findings and the critical-route gate remains 10/10 connected;
- this is not final-route acceptance. Eighty-six ratsnest items remain: 84 are
  ground/thermal stitching work and the two non-ground items are the deferred
  `P5V_REG` and `VBUS_PD` zone-island joins. There are 33 starved-thermal
  reports. These move together into the power/ground stitch stage and may not
  be closed with thin control traces or waived by this checkpoint;
- `route_usb_v2.yaml` and inherited canonical `route.yaml` remain stale until
  the settled signal prefix is authenticated and the final stitch wave is
  consolidated. Regenerated r0 SHA256 is
  `59cdef3d71f72f1c35894f3b39e7522439682768bfd01f59d07d13b46db6426b`.

Reflection: exact pad-signature replay saved the bulk of this stage. Time was
lost by discovering service-part reachability during copper authoring and by
one intermediate DRC run without matching rule sidecars; both checks belong
earlier. The next stage is deliberately narrow: engineer the two high-current
island joins, complete ground/thermal stitching, authenticate one canonical
final route, and rerun every route/electrical gate.

## 2026-08-18 16:03 — v2 power/ground closure checkpoint

- discovered a real load-path break between the `VBUS_PD` fuse-output and
  buck-input zone lobes. The PD controller/CC field separated them even though
  both lobes used the same net. The settled path uses an eight-via F-to-B bank,
  a 2.0 mm B.Cu spine, and an eight-via B-to-F bank; no thin sense trace carries
  load current;
- moved `C_AGG_IN` into the aggregate-eFuse input zone and routed both UVLO
  taps as explicitly no-load sense branches. Adjacent `VBUS_PD_RAW/VBUS_PD`
  zones now stop at the fuse boundary rather than overlap;
- stopped the generic 18-target GND A* fallback after more than five minutes
  of full-CPU work with no per-target evidence. Direct pad rescue plus fill
  reduced 83 GND opens to 18; exact residual dogbones and filled-island bboxes
  then closed them deterministically. Final result is zero opens, zero
  starved thermals and zero hard DRC findings;
- A-VIA grades all three physical series banks: `VBUS_PD` source 8 vias/4.4 A,
  `VBUS_PD` destination 8 vias/4.4 A, and protected-5V distributor 8 vias/4.4 A,
  each against 3.0 A continuous at the cited 10 C-rise basis with no fill
  credit. Two initially counted but electrically one-sided source vias were
  rejected and the bank was re-authored before acceptance;
- the 35 um copper skeleton predicts 17.23 mV/0.036 W at 75 C for the 2.1 A
  PD input spine; protected-output predictions remain 28.23 mV left,
  48.74 mV worst modeled right, and 22.92 mV management. Package, connector,
  zone-spreading and via-barrel losses remain first-article measurements;
- exact final-prefix SHA256 is
  `aad5f3e13c4cb65850783d535443eb274693f42b25eda2881872afb0f241c1fb`.
  Critical USB routing remains 10/10 connected. The only non-library native
  DRC findings are six silkscreen-clipping warnings, intentionally deferred
  to the next silk/readability cleanup rather than hidden as route waivers.

Reflection: current-carrying continuity must be proven as an ordered graph,
not inferred from a net name or zone membership. Via capacity must count only
barrels attached on both required sides. For GND, fill-and-measure residuals
were dramatically faster and more legible than broad pathfinding. These are
recorded as IMP-171 through IMP-173.
