...
kicad-cli sch export netlist -o 06_build/netlists/crow_array_pod.net 04_kicad/crow_array_pod.kicad_sch >/dev/null
# ERC gate: 0 violations at severity-all
kicad-cli sch erc --severity-all --format json -o 06_build/drc/erc.json 04_kicad/crow_array_pod.kicad_sch >/dev/null
python3 - <<'PYEOF'
import json, sys
...
$PY "$SKILLS"/import_krt.py 06_build/route/r3.kicad_pcb \
    04_kicad/crow_array_pod.kicad_pcb 04_kicad/crow_array_pod.kicad_pcb 2>/dev/null | grep imported
...
kicad-cli pcb drc --severity-all --refill-zones --schematic-parity --format json \
    -o 06_build/drc/gate.json 04_kicad/crow_array_pod.kicad_pcb >/dev/null
python3 - <<'PYEOF'
import json
