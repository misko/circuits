...
| `R_LED1` / `R_LED2` | 680 Ω | `(3.3 − 2.0)/680` = **1.91 mA**; E24 above the 650 Ω a 2 mA target implies. **THE 2.0 V IS A PART CONSTRAINT, NOT AN ILLUSTRATION** (noted 2026-07-28): it pins the indicator to a **RED** LED. A green/blue at Vf 2.6–3.1 V through the same 680 Ω runs 0.29–1.03 mA — up to 6.6× dimmer — so a colour substitution is a different design, and the two indicators `LED_PWR` / `LED_ST` are one MPN for exactly that reason |
| `C_ESD` | 100 nF 0402 | **ADDED 2026-07-28 by ADR-0008, and it was in NEITHER dossier's power to add.** ST DocID11265 Rev 5 Figure 18 (PDF p9) puts `CBUS` = 100 nF ON `U_ESD`'s VBUS pin; the USBLC6 dossier flagged the omission ("do not let it fall between the two dossiers") and so did the USB-C one. It is what makes the pin-5 leg a LOCAL AC return: the strike loop is `I/O pad → steering diode → (internal) Transil → pin 2 → GND`, closed by this capacitor, **not** a loop that runs upstream through `F_IN` |
| `C_FLASH` | 100 nF 0402 | **ADDED 2026-07-28.** `U_FLASH` VCC (pin 8) decoupling — Raspberry Pi's own routed reference fits it (C5 in Hardware Design Figure 7, PDF p10) and the W25Q128JV dossier records the obligation. It is deliberately NOT a `keep_short` budget: the flash VCC is on the global `3V3` net, so a span rule would measure the whole rail and could only ever be waived. Discharged geometrically at placement instead |
| `R_CSPU` | 10 kΩ, `QSPI_CSN` → 3V3 | **ADDED 2026-07-28, POPULATED — a deliberate departure from Raspberry Pi's DNF.** W25Q128JV §9.3 (PDF p62) requires `/CS` to TRACK VCC on the supply ramp, and the RP2040 Hardware Design guide concedes "there is a short period of time during switch-on where the state of the QSPI_SS pin cannot be guaranteed". Their R2 is DNF because the internal 50–80 kΩ suffices *with this flash* — a BOM-cost optimisation on a mass-produced board, not an engineering objection. Populating costs **0.33 mA** of a 150 mA envelope and removes this board's ONLY unpopulated footprint (the blank-LCSC CPL class that shipped 13 rows on cooksense v1.1). Divider check against the strap: 10 kΩ ∥ internal, over `R_BOOT` 1 kΩ ⇒ `BOOTSEL_N` ≈ **0.3 V**, far below `V_IL` |
| `R_BOOT` | 1 kΩ, `QSPI_CSN` → `BOOTSEL_N` | **ADDED 2026-07-28.** The BOOTSEL strap (RP2040 §2.8.1: the bootrom checks whether the SPI CS pin is tied low and skips flash boot if so). **1 kΩ and not 0 Ω** because `QSPI_SS` is a *driven output* in normal operation and a hard short to GND through the button would be a drive-fight into the pad |
| `SW_BOOT` / `SW_RUN` | SMD tact, one MPN — **`TS-1187A-B-A-B` (C318884), JLC base** | **NO DEBOUNCE CAPACITOR ON EITHER, AND THAT IS A DERIVATION, NOT AN OMISSION.** `BOOTSEL_N` is **sampled once** by the bootrom (RP2040 §2.8.1), so contact bounce is unobservable by construction — the pin is either low at that instant or it is not. `RUN_N` is a **level-sensitive** reset with a specified Schmitt input, so bounce produces a burst of resets that ends in a reset; the failure mode a debounce cap would prevent does not exist. Adding one would also fight the internal 50–80 kΩ pull-up on the release edge. The switch's own bounce time is **not specified by the vendor** — recorded as an absence, and it is why this row states the argument rather than a number. Contact current is **41–66 µA** on `SW_RUN` (from RUN's cited pull-up) and ~0.34 mA on `SW_BOOT`, both far below any wetting figure the vendor publishes — also unpublished, also recorded |
| `SW_BOOT` / `SW_RUN` (why they exist) | — | **ADDED 2026-07-28.** `nets.yaml` declares both `BOOTSEL_N` and `RUN_N` as real nets, and a net with one pin is not a node. `SW_RUN` is what makes re-flashing possible without unplugging: the UF2 ritual is hold-BOOTSEL-then-reset, and on this board "power cycle" means unplugging the USB-C cable on an instrument carrying ten torqued SMA cables. `RUN` has a specified 50–80 kΩ internal pull-up (Table 625), so a button to GND is a complete reset circuit — do **not** copy ADR-0005's external-pull-down reasoning onto it, which exists only because PE42482A-X publishes no internal pull at all |
...
| `R_LED1`, `R_LED2` | 680 Ω | §5 — 1.91 mA at **Vf 2.0 V, i.e. a RED indicator**; the Vf is a part constraint, not an illustration |
| `C_ESD` | 100 nF | §5 — ST Figure 18 `CBUS` at `U_ESD` pin 5 (ADR-0008) |
| `C_FLASH` | 100 nF | §5 — `U_FLASH` VCC decoupling (Hardware Design Figure 7, PDF p10) |
| `R_CSPU` | 10 kΩ | §5 — `/CS` tracks VCC on the ramp (W25Q128JV §9.3 p62); POPULATED, not DNF |
| `R_BOOT` | 1 kΩ | §5 — BOOTSEL strap; 1 kΩ because `QSPI_SS` is a driven output |
