...
def _run_selector(executable: str, source: Path, build_dir: Path,
    ...
    record = {
        ...
        # Record one replay-stable logical command. The absolute temporary and
        # build-directory paths used for execution are transport details, not
        # part of the generated artifact's identity.
        "command": [executable, "-o", output.name, "-D",
                    f'part="{selector}"', "-D",
...
def generate(config_path: Path, root: Path, build_dir: Path,
    ...
    try:
        config_relative = config_path.resolve(strict=True).relative_to(
            subject_root.resolve(strict=True)).as_posix()
    except ValueError as exc:
        raise EnclosureError(
            "generation config must be inside the declared subject root") from exc
