...
def _clearance_check(config_path: Path, config: Mapping[str, Any],
    ...
    try:
        ...
        replay_stable_command = (
            ...
            command[2] == installed_record.get("path") and
            command[7] == source_record.get("path"))
        legacy_absolute_command = (
            isinstance(command, list) and len(command) == 8 and
            Path(command[2]).resolve() == case_path.resolve() and
            Path(command[7]).resolve() == source_path.resolve())
...
def _physical_check(config: Mapping[str, Any], config_hash: str,
                    evidence_path: Path | None, root: Path
    ...
    try:
        evidence_relative = evidence_path.resolve(strict=True).relative_to(
            root.resolve(strict=True)).as_posix()
    except ValueError as exc:
        raise EnclosureError(
            "physical evidence must be inside the declared subject root") from exc
...
def verify(config_path: Path, root: Path, build_dir: Path,
    ...
    try:
        ...
        raise EnclosureError(
            "enclosure config must be inside the declared subject root") from exc
    config_hash = semantic_sha256(config)
    mesh_check = _mesh_check(
        config_path, config, loaded["interface"], build_dir)
    clearance_check = _clearance_check(
        config_path, config, step_report, collision_mesh,
        collision_report, collision_tolerance)
    mesh_generation = mesh_check.get("evidence", {}).get("generation_file")
    collision_generation = clearance_check.get("evidence", {}).get(
        "generation_file")
    if isinstance(mesh_generation, Mapping) and \
            isinstance(collision_generation, Mapping) and \
            collision_generation != mesh_generation:
        clearance_check["status"] = "FAIL"
        clearance_check["graded"] = max(0, clearance_check["graded"] - 1)
        clearance_check["findings"].append(
            "collision evidence binds a different generation receipt than "
            "the printable-mesh census")
    checks = [
    ...
    physical, print_verified, thermal_verified = _physical_check(
        config, config_hash, physical_evidence, root)
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--regrade-report", action="store_true",
        help="return success after emitting a valid report regardless of readiness")
