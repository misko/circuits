    ...
    def write(self, text: str) -> int:
        for line in text.splitlines(keepends=True):
            ...
            runtime_summary = tail.startswith((
                ...
                "INCOMPLETE "))
            if (self._emit_telemetry and live_telemetry) or \
                    (self._echo and not runtime_summary):
...
def run_bounded(
        ...
        max_retained_output_bytes: int | None = None) -> RunResult:
    ...
    if max_retained_output_bytes is not None and (
            isinstance(max_retained_output_bytes, bool) or
            not isinstance(max_retained_output_bytes, int) or
            max_retained_output_bytes <= 0):
        raise ValueError(
            "max_retained_output_bytes must be a positive integer or None")
    if not isinstance(emit_telemetry, bool):
        raise ValueError("emit_telemetry must be a boolean")
    ...
    console: TextIO = _LegacyConsole(
        label, echo, emit_telemetry=emit_telemetry)
    with tempfile.TemporaryDirectory(prefix="pcb-process-runner-") as directory:
        ...
        if max_retained_output_bytes is None:
            output_bytes = log_path.read_bytes()
        else:
            size = log_path.stat().st_size
            with log_path.open("rb") as stream:
                if size > max_retained_output_bytes:
                    stream.seek(size - max_retained_output_bytes)
                output_bytes = stream.read(max_retained_output_bytes)
            if size > max_retained_output_bytes:
                marker = (
                    "[process_runner: output truncated; retained final "
                    f"{max_retained_output_bytes} of {size} bytes]\n")
                output_bytes = marker.encode("utf-8") + output_bytes
