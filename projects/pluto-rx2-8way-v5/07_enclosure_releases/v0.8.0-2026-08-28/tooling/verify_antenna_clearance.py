...
FACT_KEYS = (
    ...
    "case_post_clearance_d", "case_screw_clearance_d",
    "case_web_root_d", "case_web_height", "case_web_min_throat",
    "case_web_min_section_area", "case_web_member_section_area",
    "case_web_root_member_ratio", "case_web_legacy_margin",
...
def validate_facts(actual: Mapping[str, float], expected: Mapping[str, float],
    ...
    candidate_corridor_minima = {
        "north SMA / closure web": 3.0,
        "side SMA / closure web": 3.0,
        "USB-C / closure web": 3.0,
        "top service / closure web": 3.0,
        "top service / notch": 1.5,
    }
    observed_candidate_corridors = {
        "north SMA / closure web":
            actual["north_sma_lug_candidate_clearance"],
        "side SMA / closure web":
            actual["side_sma_lug_candidate_clearance"],
        "USB-C / closure web": actual["usb_lug_candidate_clearance"],
        "top service / closure web":
    ...
    require_close(actual["case_post_clearance_d"], 9.8,
                  "case-post sleeve diameter")
    require_close(actual["case_screw_clearance_d"], 3.8,
                  "case-screw clearance diameter")
    if actual["case_web_root_d"] < 12.0 or \
            actual["case_web_min_throat"] < 12.0 or \
            actual["case_web_min_section_area"] < 85.0 or \
            actual["case_web_root_member_ratio"] < 0.8:
        raise RuntimeError("case closure structural web is below its CAD screen")
    require_close(actual["case_web_legacy_margin"], 3.0,
                  "case-web legacy-candidate margin")
