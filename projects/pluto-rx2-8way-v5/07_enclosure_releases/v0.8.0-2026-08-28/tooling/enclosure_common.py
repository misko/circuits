...
def run_bounded(command: Sequence[str | Path], *, cwd: Path | None = None,
    ...
    # Keep pure schema/config consumers independent of the process runtime.
    # External execution still resolves through the one shared authority at
    # the moment a caller actually requests a subprocess.
    local_tooling_dir = Path(__file__).resolve().parent
    release_local_runner = local_tooling_dir / "process_runner.py"
    kicad_script_dir = Path(__file__).resolve().parents[2] / "kicad-pcb" / "scripts"
    runner_dir = (local_tooling_dir if release_local_runner.is_file()
                  else kicad_script_dir)
    if str(runner_dir) not in sys.path:
        sys.path.insert(0, str(runner_dir))
    try:
        from process_runner import run_bounded as pipeline_run_bounded
    except ImportError as exc:  # pragma: no cover - deployment failure
        raise EnclosureError(
            "bounded subprocess: shared process authority is unavailable") from exc
    try:
        shared = pipeline_run_bounded(
            argv, cwd=cwd, env=env, timeout_s=timeout_s,
            heartbeat_s=min(10.0, max(5.0, timeout_s / 2)),
            label="pcb-enclosure", echo=False,
            # Report commands reserve stdout for their machine-readable
            # result. The bounded retained tail below remains available on a
            # replay failure without live runtime telemetry corrupting JSON.
            emit_telemetry=False,
            max_retained_output_bytes=max_output_bytes_per_stream)
    except (OSError, ValueError) as exc:
        raise EnclosureError(f"bounded subprocess could not start: {exc}") from exc

    # The shared adapter reads at most this retained tail from its exact log;
    # do not materialize an unbounded child transcript in this process.
