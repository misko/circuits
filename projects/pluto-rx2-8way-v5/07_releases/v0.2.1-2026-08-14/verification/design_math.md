...
F.Cu, with zero RF vias and zero intentional stubs. Seven paths use 14 native
tangent arcs; the two direct paths remain straight. The smallest realized arc
radius is 0.9883 mm, or 3.350 times the 0.295-mm routed width, against the
blocking 3.0-width minimum.

## Via geometry and process separation

U1's exposed RF-ground pad uses nine 0.45/0.25 mm filled-and-capped vias. The
remaining 615 vias use 0.45/0.20 mm and receive no fill or cap treatment. At
...
the connector is power-only. J12 adds a non-isolated alternate 4.75–5.5 V
bench input at `VBUS_RAW`; J1 and J12 must be used one at a time. There is no
electrical path that intentionally supplies or back-powers the Pluto.
