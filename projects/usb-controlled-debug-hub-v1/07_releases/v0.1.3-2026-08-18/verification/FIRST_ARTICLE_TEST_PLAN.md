...
1. Inspect both sides for bridges, tombstones, missing parts, connector seating,
   ...
   TPS2557 PowerPADs and USB2517 exposed pad are soldered.
2. Install exact Keystone 3568 at `F_IN` and Littelfuse `0297004.WXNV` 4 A fuse.
3. With all cables removed, record settled resistance using the exact probes
...
4. Confirm upstream USB VBUS is not continuous with `P5V_RAW` or
...
all exposed-pad confirmations, explicit units/probes, bench voltage/current
limit and measured values. Begin at 5.10 V with a 0.30 A maximum current limit.
Run `first_article_check.py`; continue only on `AUTHORIZED`. Record no-load
`P5V_RAW`, `P5V_PROTECTED`, `3V3_MAIN`, input current and component
temperatures. Stop on current-limit entry, odor, discoloration, oscillation,
unstable voltage or unexpected heating.
