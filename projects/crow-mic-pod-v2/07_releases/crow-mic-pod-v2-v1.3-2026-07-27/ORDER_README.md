# ORDER README — crow-mic-pod-v2 (remote microphone POD, board a) — v1.3

## ✅ v1.3 SUPERSEDES v1.2 — BOM LEGIBILITY ONLY, NO COPPER CHANGE

**v1.2 is not wrong and is not DO-NOT-ORDER.** Its board is this board: the
`.kicad_pcb` is md5-identical (`c7b8512ccf0810997116c8c2e59dcad9`) to v1.2's and
to `04_kicad/`'s, the gerbers and drills RE-PLOT from this release's own source
13/13 byte-identical after the timestamp strip, and `fab/cpl.csv` is
byte-identical. **Only `fab/bom.csv` changed, and only in its Comment and MPN
columns.**

**What was wrong with it.** Graded the way the RECIPIENT parses it (canon
F-LEGIBLE, ADR-0006), v1.2's BOM has **21 findings**:

| check | findings on v1.2 | what JLC saw |
|---|---|---|
| F-MPN | 15 | **every coded row ships a blank MPN** → JLC's matcher leaves a code-only line at *No Part Selected* |
| F-WORDS | 5 | the Comment is an LCSC code (`C192421`, `C1972959`, `C22359707`, `C2480`, `C559105`) — a row no human on either side can review |
| F-ENCODE | 1 | `Ω` with **no UTF-8 byte-order-mark**, so a cp936 reader renders `CE A9` as `惟` |

**This release: 0 findings.** 15/15 coded rows carry an MPN resolved from
`02_parts/<MPN>/part.yaml` or the vetted passives ledger, 15/15 Comments read as
real values or part numbers (`OPA1678IDR`, `CMT-8504-100-SMT-TR`, `SMAJ6.0A`,
`SS14`, `TPD2E2U06DRLR`, `RJHSE-5384`), and the file carries the byte-order-mark.

**The 15 rows are now GENERATED, not hand-trimmed.** v1.1 removed the MK1 and J1
rows by editing `fab/bom.csv` directly. Nothing in `03_src/` recorded that, so a
fresh export of this same sealed board produced **17** rows — the next person to
regenerate would have silently re-shipped the very defect v1.1 was cut to fix.
The decision now lives in `03_src/rules/assembly.yaml` as `on_bom: false` on both
entries. **The decision has not changed**: JLC is asked neither to place nor to
source MK1 or J1, exactly as section 3 below says.
...
```
bom_legibility_check.py 07_releases/crow-mic-pod-v2-v1.3-2026-07-27/fab/bom.csv \
                        --echo <saved-jlc-table>.csv
```

A code JLC redirects is a **SUBSTITUTION and a FINDING**, not a convenience. The
15-line worklist is `verification/bom_echo_gate.txt`. This is a SECOND pre-order
human gate, beside — not instead of — the A-POL pin-1 preview check in section 3b
and the J1 continuity backstop in section 2.
