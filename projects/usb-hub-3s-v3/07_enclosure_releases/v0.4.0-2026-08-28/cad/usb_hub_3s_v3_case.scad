...
// The service roof stops at the exact PCB outline.  Four localized tapered
// ears reach the closure posts, away from the J1-J5 connector banks.  Each
// ear uses an inboard root pad rather than a tangent/sliver union so roof
// twist and lid removal load cross a declared net section before the screw
// lug.  D10.8 is also the largest circular lug that remains inside the
// existing base envelope at the four C1-C4 axes.
roof_main_size = board_size;
closure_lug_d = 10.8;
closure_root_pad_d = 16.0;
closure_root_throat_min = 10.0;
closure_root_section_area_min = 32.0;
closure_head_bearing_land_min = 2.0;
closure_lug_candidate_corridor_min = 1.2;
overall_z = inside_top_z + roof;

closure_root_throat = min(closure_lug_d, closure_root_pad_d);
closure_root_section_area = closure_root_throat * roof;
closure_head_bearing_land = (closure_lug_d - screw_head_d) / 2;
closure_through_ligament = closure_lug_d - screw_clearance_d;
closure_lug_base_envelope_overhang = max(
    abs(case_holes[0][0]) + closure_lug_d/2 - outer_size[0]/2,
    abs(case_holes[0][1]) + closure_lug_d/2 - outer_size[1]/2
);
...
assert(closure_root_pad_d + 1e-6 >= closure_lug_d,
       "closure root pad is narrower than the screw lug");
assert(closure_root_throat + 1e-6 >= closure_root_throat_min,
       "closure-ear root throat is below the declared load-path minimum");
assert(closure_root_section_area + 1e-6 >=
       closure_root_section_area_min,
       "closure-ear root section is below the declared load-path minimum");
assert(closure_head_bearing_land + 1e-6 >=
       closure_head_bearing_land_min,
       "closure lug leaves too little radial land around the screw head");
assert(closure_lug_base_envelope_overhang <= 1e-6,
       "closure lug projects beyond the existing base envelope");
...
echo(str("CLOSURE_LOAD_PATH_CENSUS joints=", len(case_holes),
         " transition=convex_hull_taper",
         " print_orientation=top_face_down",
         " root_pad_d_mm=", closure_root_pad_d,
         " root_throat_mm=", closure_root_throat,
         " root_section_area_mm2=", closure_root_section_area,
         " through_ligament_mm=", closure_through_ligament,
         " head_bearing_land_mm=", closure_head_bearing_land,
         " base_envelope_overhang_mm=", closure_lug_base_envelope_overhang));
...
module roof_profile_2d() {
    union() {
        ...
        for (point = case_holes) {
            ...
            sy = point[1] < 0 ? -1 : 1;
            hull() {
                translate([sx*(board_size[0]/2-closure_root_pad_d/2),
                           sy*(board_size[1]/2-closure_root_pad_d/2)])
                    circle(d = closure_root_pad_d);
