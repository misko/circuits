# ORDER README — usb-hub-3s-v3 **v1.8** (internal board name `usb_hub_3s_v2`)
...
**NOT a USB hub, NOT USB-PD.** Release **v1.8-2026-07-26**.
Board **130.1 x 92.1 mm**, **4 layer**, **122 BOM parts / 119 CPL placements**.

> ## v1.6 AND EVERY EARLIER RELEASE ARE SUPERSEDED
>
> **v1.7 is a VERIFICATION-COMPLETE supersede of v1.6-2026-07-26. THE FAB PAYLOAD
> IS BYTE-IDENTICAL** — `fab/bom.csv`, `fab/cpl.csv`, the 13-file gerber zip and
> both drill files are the same bytes, verified. **The board is not changed and
> v1.6's board was not wrong.**
>
> v1.6 shipped 13 verification files where v1.5 had 34. The MANIFEST asserted DRC
> 0/0/0, twin 119/119, passives 26/26, stock and freshness — while the release
> carried no `drc.json`, no `erc.json`, no `bom_source_check.txt`, no stock check,
> and, worst, no `manifest_selfcheck.txt`, the artifact whose whole job is proving
> the manifest's PROSE matches its MACHINE EVIDENCE. The release asserted its own
> gate results with the evidence stripped out.
>
> **AND THE MISSING EVIDENCE HID A REAL DEFECT.** v1.6's `source/fp-lib-table`
> was copied raw from `04_kicad/` and points at `${KIPRJMOD}/../03_src/lib/...`,
> which does not exist inside the archive. Extracted on its own, v1.6's archive
> produces **12 `lib_footprint_issues`** (DRC) and **12 `footprint_link_issues`**
> (ERC). v1.5 rewrote that table to `${KIPRJMOD}/`; v1.6 did not. The gate that
> catches exactly this is `standalone_archive_drc.json` — one of the 21 files
> v1.6 was missing. v1.7 fixes the table and ships the proof: standalone archive
> DRC **0/0/0**, extracted to a bare directory with no project around it.
>
...
Surfaced by the v1.8 zero-context pin review, and stated here because no previous
document said it this directly. Every earlier write-up framed R42 as landing the
rail *on* U12's 5.25 V V_RWM **if fitted**. The corollary was never written down:
...
5.25 V V_RWM, continuously.**

What that does and does not mean:

* It is **below breakdown** (V_BR min 6.0 V), so the failure mode is **elevated
  leakage, not damage**. This is not a part being destroyed.
* It is nonetheless **operation above a datasheet rating on the configuration that
  ships**, and that is a decision, not an accident. Q9 is the gate that closes it
  with a measurement instead of an assumption.
* **Fitting R42** (160 kΩ 0402, supplied loose with the order) moves the rail to
  5.249 V and removes essentially all of the exceedance, at a cost of 102 mV of
  delivery margin — affordable only because the load is a Pi 4 at 3 A.
* The pin review's own recommendation, recorded verbatim rather than paraphrased:
  *"Recommend either populating R42 by default or swapping U12 for a 6 V-VRWM array
  on the next spin."* **That is a design decision for the next revision, not a
  change made here** — populating R42 by default would put it on the CPL and change
  the fab payload, which this verification-completeness supersede must not do.

If you are ordering this board: run Q9, and if U12's leakage at temperature is not
comfortable, fit the R42 that came in the bag.
