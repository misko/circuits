# ORDER README — usb-hub-3s-v3 **v1.12** (internal board name `usb_hub_3s_v2`)
...
**NOT a USB hub, NOT USB-PD.** Release **v1.12-2026-07-28**.
Board **130.1 x 92.1 mm**, **4 layer**, **122 BOM parts / 119 CPL placements**.

## 🔴 v1.12 SUPERSEDES v1.11 — J5's LAND PATTERN WAS WRONG. COPPER MOVED.

**Order v1.12. Do not order v1.11's gerbers.** This is the first release of this
board since v1.6 whose copper features are in different places, and the reason is
a footprint defect that had been in every release since v1.0.

`J5` is the USB-C receptacle that feeds the Pi — HRO `TYPE-C-31-M-12A`, LCSC
`C5337088`. Its footprint had inherited KiCad's stock
`Connector_USB:USB_C_Receptacle_HRO_TYPE-C-31-M-12` geometry, which **does not
match the part**. Three independent sources say so and agree with each other:

| | pad length | pad row -> alignment-hole line | hole dia |
|---|---|---|---|
...
This connector **cannot self-align in reflow**. It is pinned by two moulded
Ø0.50 posts in non-plated holes and four stamped shell legs soldered into
plated slots, so surface tension on twelve lands cannot pull it into place — the
body goes where the posts and legs put it.

Measured from JLC's own 3D model, anchored on those posts (dia 0.5000 at
x = ±2.8900, span 5.7800 — exactly the sheet; housing 7.35 deep with the tail
projecting 0.4000, the sheet's own `0.40 ± 0.10`), the solderable contact tail
runs **1.070 … 1.470 from the alignment line**:
...
**The tail is 100 % on its land in both** — nothing was hanging off. What v1.11
did was **invert the fillet balance and over-paste the row**: the heel, where
...
coordinate is computed from the pad-centre bounding box — which was wrong. v1.11
told the machine to put this connector **0.2025 mm north of where its own
alignment posts sit**, against 0.075 mm of radial post clearance. Measured
against JLC's own `C5337088` package: their model origin is +1.30492 mm from the
alignment line; v1.11 emitted +1.10250; v1.12 emits +1.30500.

### What you can check yourself, in one minute

* `fab/bom.csv` is **byte-identical** to v1.11's — no part changed.
* `fab/cpl.csv` differs in **exactly one row**: `J5`, `Mid Y` -107.502 -> -107.705.
* `verification/j5_geometry_gate.txt` — 127 checks, two instruments sharing no
  code, plus two RED-verifies that make the same gate fail on demand.
* `verification/copper_xor.txt` — 28.10 mm2 of copper moved, of which
  0.000969 mm2 (0.0034 %) lies outside the J5 corner, and that remainder is
  zone-filler slivers tens of microns across.

### One thing this release does NOT fix

The alignment-hole line sits **5.600 mm** from the south board edge, where the
vendor sheet's nominal is 5.79. That is a pre-existing edge position carried
from v1.0 and this release deliberately did not touch it: moving it would move
the USB-C mouth relative to the board outline, which is a mechanical change to a
mating interface and belongs in its own release with its own review. The
connector still overhangs the edge as it always has; if you are building an
enclosure, measure from the STEP file, not from the sheet.

### The alignment holes are now Ø0.60, not Ø0.65

Both are inside the sheet's own +/-0.05 band. Ø0.60 is the vendor's nominal and
is what JLC ship in their own package. Ø0.65 was chosen originally to guarantee
that the Ø0.50 posts enter; at the corrected land pattern it no longer fits,
leaving 0.194 mm from land A1/B12 to the hole against this board's
`min_hole_clearance` of 0.200. If the posts are ever found to bind, that is the
number to revisit — it is a recorded decision, not an accident.
