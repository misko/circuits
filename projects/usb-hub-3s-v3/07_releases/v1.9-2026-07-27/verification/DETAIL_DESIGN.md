...
**STACKUP — DECLARED HERE FOR THE FIRST TIME IN v1.9** (fix-lens P2-7: the
`.kicad_pcb` and `.kicad_pro` contain **zero** `stackup` entries, so until now
the copper weight behind every ampacity figure in this project was an unnamed
fab default). **Assumed and to be ordered: JLC 4-layer 1.6 mm STANDARD — 1 oz
outer (34.8 µm) / 0.5 oz inner (17.4 µm).** The two inner planes (In1 = GND,
In2 = VIN) therefore carry **half** the copper per unit width that a "1 oz
external" figure would imply. It changes no conclusion in this document — every
IPC-2221 requirement quoted is an outer-layer one, and the only class whose
margin leans on plane copper (PWR_IN) has its 1.99× attributed to VBAT's
8.750 mm on **F.Cu** — but the weight is now pinned instead of inherited.
Declaring it in the board file is a **next-rev** work order: doing it in v1.9
would change the sealed payload the three v1.9 reviews were run against, for a
parameter the gerbers do not carry. The order-form obligation is in
ORDER_README section 2.
...
#### 2.5a **CORRECTION (v1.9) — the CS/CSG pair is NOT a Kelvin connection**

**This section used to say:** *"CS/CSG reach the shunt as a Kelvin pair through
0 Ω links (R9/R10, R18/R19) so no shared trunk copper enters the sense loop."*
**That is falsified by measurement and is withdrawn.** It was stated as a fact
about the copper, and the copper says otherwise (v1.9 red-team fix lens, P2-1;
the v1.5 layout lens saw the same thing as RL-8 and under-estimated it at 2 %).

`R10.1` — the buck-A CSG link — lands on the **GND net at (75.175, 47.000)**,
i.e. on the plane, **3.73 mm from `RS1.2`** at (72.000, 48.962), which is the
shunt's own ground terminal. (Re-measured here on the sealed v1.9 board by pad
position; the fix lens reports 3.75 mm and the v1.5 lens 3.93 mm — same tap,
different reference points on the pads.) Buck-C is identical: `R19.1` on GND at
(75.175, 85.000), 3.73 mm from `RS2.2`.

**How much shared copper that puts inside the sense loop**, by SOR mesh solve on
the GND copper (4 layers, via-coupled; source `RS1.2`, sink the five buck-A
input-cap GND pads):

| | buck-A | buck-C |
|---|---|---|
| input-return resistance, shunt GND → Cin GND | 1.002 mΩ | 1.012 mΩ |
| potential at the CSG tap, as a fraction of that drop | 0.6418 | 0.6239 |
| **shared trunk copper inside the sense loop, CSG side** | **0.359 mΩ** | **0.381 mΩ** |
| shared copper, CS side (`R9.1` / `R18.1`) | 0.124 mΩ | 0.174 mΩ |
| **total sense-loop error resistance** | **0.483 mΩ** | **0.555 mΩ** |

**What it costs, and the corrected limit.** The error adds to the shunt signal,
so the controller sees a larger drop than the shunt actually produces and trips
**early**:

    buck-A at 6 A: 0.483 mOhm x 6 A = 2.90 mV on a 60 mV full-scale signal
                   = +4.8 %  ->  I_LIM 11.0 A becomes ~10.5 A
    buck-C at 5 A: 0.555 mOhm x 5 A = 2.78 mV on 50 mV
                   = +5.6 %  ->  I_LIM 11.0 A becomes ~10.4 A

**≈10.5 A is the number to use**, not the 11.0 A of eq. (10). It is still
**1.75×** the 6 A rail load and still clears TI eq. (11), so nothing about the
design changes — but the sentence that was there claimed a property the board
does not have, and a claimed Kelvin connection is exactly the kind of thing a
later reviewer would build on.

**Two further honest notes.** The shared path is not only resistive: it carries
the buck's *pulsed* return, so it is inductive as well, and there is **no
differential filter** — R9/R10/R18/R19 are 0 Ω links and the netlist contains
**no capacitor between `CSF_x` and `CSGF_x`**, although the 0 Ω links were
clearly provisioned for one.

**NEXT-REV WORK ORDER** (not done in v1.9 — it is a re-route, and re-routing
voids every verdict this release collected): run CSG to `RS1.2` / `RS2.2`'s own
pad as a dedicated trace, and fit the CS RC filter the 0 Ω links exist for.
...
### 2.10a DEMB — **R_DEMB = 0 Ω, and what that actually chooses** *(added v1.9)*

`U2.11` and `U11.11` (DEMB) are tied **directly to GND**, i.e. R_DEMB = 0 Ω.
This file's own rule is *"a value in the schematic with no line here is
UNJUSTIFIED"*, and until v1.9 it contained **zero occurrences of "DEMB" or
"diode emulation"** — a departure from the very TI worked design sec.2 declares
it adopts (Figure 7-1 / Table 7-1 fit **R8 = 10 kΩ** from DEMB to PGND), with
no written derivation. Raised by the v1.9 zero-context pin review (PR-4).

SNVS499I states it plainly: *"When R_DEMB = 0 Ω, the LM5116 will always run in
diode emulation"*, and *"Fully synchronous operation is obtained if the DEMB pin
is always biased to a higher potential than the SW pin when LO is high.
R_DEMB = 10 kΩ will bias the DEMB pin to 0.45 V minimum."*

**What it means on this board.** Both bucks run in **permanent diode emulation**
and never operate fully synchronously. The boundary, re-derived:

    I_pp = (V_OUT / (L x f_SW)) x (1 - V_OUT/V_IN)
         = (5.15 / (6.8 uH x 250 kHz)) x (1 - 5.15/12.6) = 1.79 A
    DCM below I_out ~ I_pp/2 = 0.9 A per rail

**It is a legal and, here, a defensible choice — but it is a CHOICE.** Grounding
DEMB is the datasheet-sanctioned configuration, it is what permits start-up into
a pre-biased output without sinking current backwards through the load, and DCM
at light load is the right efficiency posture on a battery-powered board that
spends most of its life lightly loaded. What it costs is fully-synchronous
operation at light load and a slightly less well-behaved small-signal response
across the DCM/CCM boundary — both of which fall inside what **bench gate Q3**
already scopes (SW node through startup, shutdown and 0→3 A→0 load steps).

**RECORDED, NOT CHANGED, in v1.9** — fitting a 10 kΩ DEMB resistor is a BOM and
board change, and this release changes no copper. Next-rev decision item.
...
**CORRECTION (v1.9) — that sentence cites a ceiling 5VA's own worst corner
already exceeds.** Raised as P2-8 by the v1.9 red-team fix lens, and
independently by the v1.9 render review (finding 3) from the other side of the
same fact. The argument above is *"5VA is not raised, because the USB-A window
ceiling is 5.25 V and a proportional bump would push its no-load corner to
5.35 V — over the port limit."* **sec.4's own table gives 5VA's worst max as
5.273 V**, which is already **+23 mV over 5.25 V**, so the reason given for not
raising the rail is a limit the rail does not meet. Re-derived here rather than
taken from the review:

    1.215 x 1.015 x (1 + (3.92 x 1.001)/(1.21 x 0.99)) = 5.2731 V

**ADJUDICATION — the CEILING is right, the CORNER is right, and this is a real
(small) margin finding.** USB 2.0 §7.2.1 does require a self-powered hub's
downstream port to supply **4.75–5.25 V at the connector**, so 5.25 V is the
correct ceiling and 5.273 V is the correct corner; they simply were never put
side by side. Bounding it:

* it is the **no-load, all-tolerance-extremes** corner (Vref +1.5 % **and**
  R3 +0.1 % **and** R4 −1 %) — under any real port load the IR drop of sec.4's
  5VA budget (≥11 mV at the pour alone) pulls the receptacle back inside;
* the exceedance is **23 mV = 0.44 %**, against a 5 % USB tolerance band;
* the same 5.273 V is what `DETAIL_DESIGN` sec.5.3 already dispositions on the
  **USBLC6 side** (U8/U9/U10's 5.25 V characterised point). **Nothing caught the
  USB-2.0-spec side**, which is the actual finding here: one number, two
  independent limits, only one of them checked.

**NO HARDWARE CHANGE IS WARRANTED** for a 23 mV no-load corner, and lowering
5VA would spend the +151.8 mV E-MARGIN slack sec.4 now computes for it. R42
exists as a DNP setpoint-trim strap on **5VC**, not on 5VA, so it is not a lever
here. RECORDED so the two sentences stop contradicting each other; if a future
revision wants the corner inside 5.25 V the lever is R4/R13 at ±0.1 % — that
alone takes the max to `1.215×1.015 × (1 + (3.92×1.001)/(1.21×0.999))` =
**5.237 V**, inside the window — not a setpoint change.
...
> **CITATION CORRECTION, v1.9 (pin review PR-8).** The claim that U7's unused
> channel-2 pins may float has been carried by both this file and
> `02_parts/TPS2513ADBVR/part.yaml` **with no datasheet line behind it**, and the
> only *"can be grounded or left floating"* sentence in SLVSBY8D belongs to the
> **TPS2514x** pin table, where pins 3/4 are genuine `N/C`. **That is a different
> device variant.** On the dual **TPS2513A** that is fitted, pins 3 and 4 are
> real `DP2` / `DM2` detection I/O. The claim is therefore an **engineering
> judgement, not a datasheet permission**, and is restated as one: an unused
> BC1.2 detection channel with nothing attached to it has no port to
> characterise and idles — the detection state machine has no load to respond
> to. Risk assessed as low; the honest label is "unqualified by the datasheet",
> and grounding DP2/DM2 or fitting the single-channel TPS2511/TPS2513 for the
> third port is a next-rev tidy, not a defect.
...
> **2 A AND 2.5 A ARE LOAD BUDGETS. THE HARDWARE CEILING IS 2.72 A.** (v1.9
> fix-lens P2-5, and the number *"appeared nowhere"* before this line.) The
> TPS2557 is guaranteed **not to limit below I_OS(min) = 2.72 A**, so 2.72 A is
> what a port can carry indefinitely and **nothing on the board enforces 2 A or
> 2.5 A**. Three ports at that ceiling is **8.16 A** on a rail budgeted at 6 A —
> and 8.16 A is still below buck-A's 11.0 A valley limit (≈10.5 A corrected,
> sec.2.5a), so **nothing intervenes**: the case is reachable, not merely
> unlikely, and it has to be survivable rather than improbable.
> **It is survivable, term by term:** L1 I_rms 10 A / I_sat 15.2 A ✓;
> RS1 `8.16² × 10 mΩ` = **0.67 W** in a 1 W 2512 ✓; F1 ≈7.2 A of a 10 A blade
> (the C rail is 3 A now) ✓. The cost lands on the **VBUS feed**: 2.72 A on the
> 0.800 mm × 13.554 mm B.Cu run is **ΔT 19.4 °C** and wants 1.132 mm (IPC-2221,
> 1 oz external), against 9.6 °C at 2.0 A and 16.0 °C at 2.5 A. Over the house
> ΔT of 10 °C, acceptable on a trace heat-sunk at both ends, **measured by bench
> gate Q4b**, and the recorded v-next widening is sized on 2.72 A (≈1.15 mm),
> not on 2.5 A. Full derivation in `03_src/rules/nets.yaml`, class `VBUS`.
...
**WHICH 5.25 V APPLIES TO WHICH CORNER — adjudicated v1.9** (render review
finding 3 asked for 5VC's 5.479 V to be recorded as "229 mV over the port
limit… for every attached device"; that is right in spirit and cites the wrong
limit for a Type-C port, so it is settled here rather than repeated):

| corner | the limit that actually applies | verdict |
|---|---|---|
| **5VA 5.273 V** at the USB-A receptacle | **USB 2.0 §7.2.1: 4.75–5.25 V** at a self-powered hub's downstream port | **+23 mV OVER.** Real, small, no-load-only — sec.2.11 |
| **5VC 5.479 V** at the USB-C receptacle | **USB Type-C `vSafe5V`: 4.75–5.5 V** for a source — *not* the USB 2.0 downstream-port number | **INSIDE, by 21 mV.** A Type-C source is permitted 5.5 V |
| 5VC 5.479 V **as seen by the Pi 4** | Pi 4 recommended input **5.0 V ±5 % = 5.25 V**; absolute max **6.00 V** | **+229 mV over RECOMMENDED, 521 mV under ABSOLUTE MAX** |
| 5VC 5.479 V / 5VA 5.273 V **at U12 / U8-U10** | USBLC6-2SC6 V_BUS **characterised** at 5.25 V; V_BR min 6.0 V | already dispositioned — sec.5.3, ORDER_README gate Q9 |

**So the honest statement is narrower than "over the port limit" and worse than
"only U12":** the C rail is inside the Type-C source window, but at the no-load
all-tolerance-extremes corner it sits **above the Raspberry Pi 4's own
recommended-input maximum** while staying well under its absolute maximum. That
is a *supervised-prototype* posture, it is consistent with ADR-0002's
best-effort OV story, and it is what **R42** (DNP, supplied loose) exists to
retire if bench gate **Q9** says so. No hardware change in v1.9.
...
`03_src/rules/power_tree.yaml`:
    ...
    GND return J5 -> buck-C  0.956 mOhm MEASURED 2026-07-26 (was ABSENT, not
                                       small: a term missing from a budget is
                                       indistinguishable from a term that is 0)
    USB-C connector          ~5 mOhm  (VBUS pins A4/B4/A9/B9 in parallel)
    0.3-0.5 m 5 A cable      ~45 mOhm  (REQUIRED, ORDER_README section 5)
    -------------------------------------
    total                    ~98 mOhm  ->  IR at 5 A = 490 mV
                                       ->  IR at 3 A = 294 mV   <== THE REAL LOAD
    ...
    5 V / 3 A. Re-grading E-MARGIN with EVERY OTHER NUMBER UNCHANGED (at the
    98 mOhm that includes the GND return; the 97 mOhm forward-only figures were
    +15.0 mV at 5 A and +247.8 mV at 3 A):
        5 A: 98 mOhm x 5 A x 1.2 = 588.0 mV -> 4.639 V ->   +9.0 mV slack
        3 A: 98 mOhm x 3 A x 1.2 = 352.8 mV -> 4.874 V -> +244.2 mV slack
    ...
    headroom      = V_5VC(min) − V_UV(Pi) = 5.227 − 4.63       = 597.0 mV
    raw IR drop   = 98 mOhm x 3 A (the Pi 4's load, ADR-0004)  = 294.0 mV
    E-MARGIN need = raw IR x 1.20 derate (power_topology.py)   = 352.8 mV
                                              ->  PASS, slack = +244.2 mV
...
**THE NUMBER IN FORCE IS +244.2 mV.** `03_src/rules/power_tree.yaml` declares
`ir_budget_mohm: 98` (the 97 above **plus** the 0.956 mΩ GND return that the
forward-only solve had silently omitted — a term missing from a budget is
indistinguishable from a term that is zero), and the load is the Pi 4's 3 A:

    headroom      = 5.227 - 4.63                          = 597.0 mV
    E-MARGIN need = 98 mOhm x 3 A x 1.20                  = 352.8 mV
                                              ->  PASS, slack = +244.2 mV

Four figures have been published for this one margin, and **every one of them
was correct for its own load and its own copper model** — the history is kept
because each step removed an optimistic assumption and *none of them changed
the hardware*:

| slack | corner | board copper | load |
|---|---|---|---|
| 157 mV | Vref-only, no derate | 3 mΩ guessed | 5 A |
| 69 mV | tolerance-inclusive + 1.20 derate | 3 mΩ guessed | 5 A |
| 15 mV | same | 12 mΩ MEASURED (RL-2) | 5 A |
| **+244.2 mV** | same | 12 mΩ + 0.956 mΩ return | **3 A (the Pi 4 — ADR-0004)** |

`power_topology.py` grades `headroom >= ir_budget × (1 + margin)` with `margin`
defaulting to **0.20**, so 352.8 mV is the number that decides the gate.
**"15 mV of paper slack is not a margin to ship on" was true and is no longer
the situation** — but a computed margin is still a computed margin, and gates
Q2/Q5 measure it.

**OPEN, UNRECONCILED (v1.9):** the v1.9 fix lens ran its OWN SOR solve over the
same three delivery segments and got **3.009 mΩ** against RL-2's **9.32 mΩ** —
a 3× disagreement between two mesh solves on identical copper, which neither
side could explain. It is in the **safe direction** (the shipped budget is the
pessimistic one; if 3.009 is right the slack is +276.6 mV), which is the only
reason it is not a finding. It is recorded rather than papered over, in
`power_tree.yaml` beside `ir_budget_mohm`, and it is settled by Q2/Q5 on the
bench, not by whichever number reached the file first.

### 4a — E-MARGIN on the **5VA / USB-A** rail (ADDED v1.9; never previously computed)

Raised as P2-6 by the v1.9 fix lens: E-MARGIN ran only on 5VC, while 5VA feeds
**three known 2 A loads** through a switch, a connector and 13.554 mm of
0.800 mm track. A regulated rail feeding a known load with no margin check is
the exact condition the gate exists to make impossible.

**Load floor: 4.75 V at the connector** — USB 2.0 §7.2.1, a self-powered hub's
downstream port. That is a *spec point*, not a brownout knee; it is what this
board owes at its own receptacle.

    headroom = 5.032 (worst-low 5VA, sec.4 table) - 4.75 = 282.0 mV

Delivery path, worst port (U5 → J4), **each term at the current that term
actually carries**:

| term | value | drop |
|---|---|---|
| 5VA pour `L1.2 → U5.2/3` — MEASURED (SOR), charged at the full 6 A | 1.895 mΩ | 11.4 mV |
| TPS2557 rDS(on), max over −40…125 °C (SLVS931B) | 35 mΩ @ 2 A | 70.0 mV |
| VBUSA feed: 13.554 mm × 0.800 mm B.Cu (9.71 hot) + F.Cu bond (0.64) + 3 vias ∥ (0.17) + port pour (≈1.5) | ≈12 mΩ @ 2 A | 24.1 mV |
| board GND return (same class as the MEASURED 1.002 mΩ input return) | ≈1.5 mΩ @ 2 A | 3.0 mV |
| **worst-port total** | | **108.5 mV** |

`power_tree.yaml` declares `ir_budget_mohm: 18.1` on this rail, and **that is an
EFFECTIVE rail-level resistance, not a resistance between two points** — three
of the four terms are per-port at 2 A and only the pour carries 6 A. It is
defined so the gate's own arithmetic reproduces the worst-port drop:
`108.5 mV / 6 A = 18.1 mΩ`.

    E-MARGIN need = 18.1 mOhm x 6 A x 1.20 = 130.2 mV against 282.0 mV
                                  ->  PASS, slack = +151.8 mV
    delivered at the receptacle: 4.923 V

**Stated because it is what a user actually meets:** charging the two USB-A
**mating contacts** (30 mΩ max each, USB 2.0) to the board takes the drop to
228.5 mV → 274.2 mV derated → **PASS with +7.8 mV**, delivered 4.804 V. At the
**2.72 A hardware ceiling** of sec.3 the receptacle sits at 4.885 V, still
≥4.75 V (4.722 V past the contacts). **Not this board's budget, but stated so
nobody is surprised:** a 1 m 28 AWG USB-A cable is ≈380 mΩ round trip = **760 mV
at 2 A**, putting the device end near 4.04 V — below USB 2.0's 4.40 V function
minimum. That is a property of cheap cables. The 5VC budget carries its cable
because this project specifies the cable; the USB-A cables are the user's.

**Cable-end estimate at the load that actually ships.** At the Pi 4's 3 A the
un-derated paper corner is `5.227 − 0.294 =` **4.933 V**, 303 mV above the Pi's
4.63 V undervoltage trip, so Q5's floor is **≥ 4.90 V** — still deliberately
above the arithmetic. **The paper margin is not the gate**; ORDER_README Q2/Q5
measure VBUSC at the board and at the far end of the actual cable, hot, at 3 A.
HISTORICAL, at the mistaken 5 A Pi-5 load: the corner was `5.227 − 0.490` =
**4.74 V** and Q5's floor (≥4.80-4.85 V) sat *above* it **deliberately**,
because the estimate is the
...
ports, **U12 on the USB-C port**. Pins 1 and 6 are both `I/O1` and pins 3 and 4
both `I/O2` (ST doc 11265 Rev 5, Fig. 1) — **each pair is ONE internal node**,
GND on pin 2, and the V_BUS transil on pin 5 tied to that port's VBUS rail.

> **PRECISION, v1.9 (pin review PR-5).** This used to read *"pass-through pairs
> 1-6 (D+) and 3-4 (D−)"*, which implies a property the netlist cannot carry.
> Because each pair is one internal node, `U8.1` and `U8.6` are on the SAME net
> (`DP_A1`), and once both pads are on one net **nothing in the netlist, the ERC
> or the DRC can distinguish an in-line clamp from a stub hanging off a tee.**
> The property that makes an ESD array useful — that it is the first thing a
> transient meets, between the connector and everything downstream — is a pure
> **LAYOUT** property on this board, with no gate behind it. Stated as a layout
> assertion, not as a wiring fact. A routing check for it is a gate gap reported
> upstream, not something this release invents locally.
