...
(117 mW, 39 % of the SOT-23 300 mW rating). The machine-readable form of that
derivation, and the two bounds a topology check cannot see, live in
`03_src/rules/power_tree.yaml` + `02_parts/ME6211C33M5G-N/part.yaml`
(`dropout_mv:`, `pdiss_max_mw:`); **E-TOPO grades all three and PASSES — see
the now-closed O8 row in §12.**
...
- **SMA bottom-plane antipad ≥ Ø3.5 mm**, opening OUTWARD toward the 5.08 mm
  ...
  over solid copper; opening Ø2.6 → Ø3.5 buys 5.6 dB of return loss at 6 GHz
  for free (RL 8.9 → 14.5). This rule was originally derived BACKWARDS and would
  have been frozen into the footprint; the *size* of its prize was separately
  overstated as ~9 dB until 2026-07-29. ADR-0007.
...
| `PWR` | `VBUS`, `VBUS_PF`, `VBUS_F`, `3V3`, `3V3_SW`, `VREG_VOUT` | 0.10 A envelope. `VBUS_PF` is the node BETWEEN the polyfuse and the ferrite (four series elements on the USB entry, not three) |
...
| **O5c** | ~~Mid-value YAT stock is UNVERIFIED~~ — **QUERIED 2026-07-28, and the answer is now MEASURED: YAT-15A+ `C7169783` 79 · YAT-12A+ `C5839322` 4 · YAT-5A+ `C6338032` 10 · YAT-3A+ `C5205332` 152** (against the committed YAT-10A+ 150 / YAT-2A+ 103). **Only YAT-15A+ has usable stock**; 12A+ and 5A+ are effectively unbuyable. A `15 + 10` pre-split pad would drop the per-board YAT-2A+ count 5 → 2, lifting the binding ceiling from **20 boards to ~51**, and delete ~12 mm of interconnect | **NOT TAKEN at stage 4, and the reason is ADR-0016's own method:** the ≥40 dB guarantee is built on datasheet MIN columns and **YAT-15A+'s min column has not been read**. Substituting it re-derives DETAIL_DESIGN §3.4 and moves ADR-0016's arithmetic — a stage-1/3 backtrack, not a schematic edit | **user decision** (costed, not silently taken) |
| ~~O5d~~ | ~~**The 12 MHz crystal is deliberately not selected.**~~ **CLOSED 2026-07-28 by taking ADR-0012 resolution (a): `ABM8-272-T3`, JLC `C20625731`, 17 567 in stock.** It is CL = 10 pF / ESR = **50 Ω max**, exactly Raspberry Pi's stated limits, and its own datasheet p.(2) reads *"Crystal approved for use with Raspberry Pi's RP2040 and RP235x range"*. So the schematic carries the reference circuit **unmodified** — 2 × 15 pF (derived: `2 × (10 − 3) = 14 → E24 15`) and the 1 kΩ damping resistor — and **no start-up test at temperature extremes enters the release gate**, which resolution (b) (the Basic 20 pF C9002, 33 pF caps) would have required. Cost, stated: Extended-library status and **zero ESR margin** — 50 Ω max against a ≤50 Ω limit, bought back only by 50 Ω being the value the vendor's own 1 kΩ damping resistor was sized against | nothing | **CLOSED** |
...
| ~~O8~~ | ~~**E-TOPO cannot express a linear regulator.**~~ **CLOSED 2026-07-27 IN `skills/`, VERIFIED HERE 2026-07-28.** The row above was true when it was written and is not true now: `power_topology.py:normalize_type()` gained a **`LINEAR`** return for a linear pass element, and the module's own docstring names this board as the case that forced it (*"`pluto-cal-switch` declared its LDO rail truthfully instead, took the exit 2, and reported the gap — which is how it got fixed"*). **What the gate now DOES, measured at this board's schematic gate:** `E-TOPO OK: 1/1 rail(s) topology-correct, covering 1/1 converter part(s)`. **What this board's LDO rail grades as:** `required=BUCK, declared=LINEAR (ME6211C33M5G-N) -> step-down requirement MET by a linear pass element`, then the two failure modes the Vin-vs-Vout derivation is blind to — **headroom 1034 mV** (Vin_min 4.40 − Vout_max 3.366) against **dropout 120 mV**, and **PD 202 mW** ((5.25 − 3.234) × 0.10 A) against a **300 mW** SOT-23 rating (**67 %**) — both **PASS**. `LINEAR` is not a fourth topology: it is one *implementation* of the step-down requirement, so the over-engineering verdict still fires unchanged. Note what closed the gap: **declaring the rail truthfully and taking the exit 2**, rather than deleting `power_tree.yaml` (which returns N-A and exit 0 — the M-COVER silent-skip class three other fleet boards took). The two numbers that now grade it were already in this repo as prose nothing could read; they are `dropout_mv:` and `pdiss_max_mw:` fields in `02_parts/ME6211C33M5G-N/part.yaml` | nothing | **CLOSED** |
| ~~O8b~~ | ~~**E-ADR demands invariants from a SUPERSEDED ADR.**~~ **CLOSED 2026-07-28 IN `skills/`, RE-MEASURED HERE 2026-07-29: `E-ADR OK: 10/10 protection/topology ADR(s) cited by an invariant`.** The row below was true when written. `electrical_invariants.py:protection_adrs()` now READS the front-matter `status:` — via a dedicated `_adr_status()` that parses only the YAML front matter, because the word "superseded" appears in ADR PROSE often enough that a whole-file grep was itself a defect (the S-VER failure mode verbatim) — and skips any ADR whose status begins `superseded`. The denominator therefore fell from 12 to 10 (ADR-0006 `superseded-by-0015`, and one more superseded-tagged ADR) and the numerator with it, so the gate is GREEN with nothing waived and nothing retagged. **NOTE WHAT DID *NOT* HAPPEN:** the ADR was never re-tagged to hide it from the checker, which is what the original declaration refused to do; the fix landed where the defect was. The stage-5 brief for this board still said to expect `11/12` and to leave the row red — this entry exists because the number was RE-MEASURED instead of assumed, and a row claiming a gate is red while the gate prints OK is the same stale-prose defect as the 18.5 mm arm separation | nothing | **CLOSED** |
| ~~O8b (original text)~~ | ~~`electrical_invariants.py:protection_adrs()` excludes only `0000-example`; it does not read `status:`.~~ ADR-0006 is now `superseded-by-0015` and its decision no longer exists, so there is no design intent left to assert — yet E-ADR reports `11/12 ... no invariant cites adr: 0006`. **The intent loop IS closed**: ADR-0015 emits 3 assertions and ADR-0016 emits 1, and they hold the current intent. Fleet check: this is the FIRST board to supersede a protection/topology-TAGGED ADR (the only other superseded ADR in `projects/`, cooksense 0005, carries no `tags:` line and so was never in the denominator), which is why nothing has hit it before. **Declared, not worked around** — the fix belongs in `skills/` and gaming it by retagging the ADR would be the kind of silent downgrade this canon exists to prevent | the E-ADR gate on this board | skills/ owner |
