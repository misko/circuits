...
| `YAT-10A+` | C5839318 | 3 | PAD DESCRIPTION table + TOP/BOTTOM VIEW pad-numbering figure, REV B p.2 |
| `YAT-2A+` | C5205333 | 2 | same, REV B p.2 |
| `0402WGF499JTCE` | C25120 | 3 | 2-terminal passive — exempt from the figure rule |
| `KH-SMA-KE-Z` | C504007 | 2 | drawing sheet 2/2 `PCB layout`, 5-Ø1.4 @ 5.08 mm |
...
part. Four entries depart from that. **Each is declared here rather than
...
|---|---|---|---|
| `SMP-MSLD-PCE-5T` | **PDF not vendored; sha256 pending.** Land pattern and RF-axis height ARE read (drawing sheet 2), but **sheet 1 of 3 is missing from the LCSC copy** and `amphenolrf.com` returns HTTP 403 to automated fetch | vendor blocks automated fetch | obtain the full 3-sheet drawing; the missing number is the **mating-face position relative to the board edge**, which sets the board-to-Pluto separation |
...
- **SMA→SMP adapters (`134-1019-451`, Cinch) are NOT on this BOM** — they screw
  onto the Pluto. But they cost **3 × $33.83 = $101**, more than this board's
  entire BOM, and only **21 are in stock**. Order early. See ADR-0006.
...
`C534203` 2448 · `C5839318` 150 (⇒ 50 boards, the binding ceiling) ·
`C5205333` 103 · `C25120` 1.78 M (Basic) · `C504007` 19 252 ·
`C6297051` 540 · `C2040` 57 220 · `C82317` 8 927 · `C82942` ~large ·
`C319160` 31 612 · `C7519` ~large.
