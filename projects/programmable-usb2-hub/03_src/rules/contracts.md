...
| `rf.yaml` | RF/high-speed applicability and exact-artifact review contract, schema 1. REQUIRED even when disabled; when enabled it binds ports, bands, impedance cross-sections, claims, first-article acceptance, and independent schematic/PCB/fab reviews. Graded by `rf_contract_check.py`. |
...
| `requirements.yaml` | D-SPEC/E-PATH external-output contract: connector count, simultaneous load, current, voltage window, duty, measurement plane, path boundary, and linked power-tree claim. Graded by `early_design_check.py` before schematic review. |
| `power_stages.yaml` | E-SWDRV controller/MOSFET gate-drive compatibility using worst-case or qualified-maximum gate charge. Graded before layout. |
| `protection_paths.yaml` | E-SURGE source/TVS/downstream coordination and transient qualification. Graded before layout. |
...
| `rails[].external_output` | `early_design_check.py` | declares whether the rail leaves the board and needs a bounded claim |
...
| `power_claims[].*` | `early_design_check.py` | ID, rail, count, simultaneous load, current, voltage, duty, measurement plane, and boundary |
| `no_external_power_outputs` | `early_design_check.py` | explicit applicability decision when no claims exist |

### keys: 03_src/rules/power_stages.yaml

| key | reader | why |
|---|---|---|
| `schema` | `early_design_check.py` | E-SWDRV schema version |
| `stages` | `early_design_check.py` | complete set of switching stages |
| `stages[].*` | `early_design_check.py` | controller capability, frequency, population, Qg, and thermal inputs |
| `no_external_gate_drive_stages` | `early_design_check.py` | explicit applicability decision when no stages exist |
...
| `paths` | `early_design_check.py` | complete set of surge-exposed paths |
| `paths[].*` | `early_design_check.py` | source, suppressor, downstream limits, margin, and qualification |
| `no_surge_exposed_paths` | `early_design_check.py` | explicit applicability decision when no paths exist |
