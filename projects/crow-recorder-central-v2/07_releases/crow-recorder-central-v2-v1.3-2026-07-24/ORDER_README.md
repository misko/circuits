# ORDER README — crow-recorder-central-v2 (8-channel CENTRAL recorder) v1.3
...
Release **v1.3-2026-07-24**. Board **170.1 × 120.1 mm, 6 layer**.

**v1.3 SUPERSEDES v1.2 — CPL ROTATION FIX (assembly data only; copper UNCHANGED).**
An external review of the sealed v1.2 (orchestrator-verified against the sealed
bytes; archived verification/external_review_v1.2.md + 08_reviews/) found
**fab/cpl.csv shipped generic footprint-name rotation-DB values that CONTRADICT
the digital twin's exact pad-fit** on 10 parts — including the consigned U1
(XU316, TQFP-128) at CPL **270°** vs the exact-fit **90°** = **180° off**. Root
cause: JLC's CPL zero-orientation is a PER-LCSC fact (how JLC drew each part's
own footprint), which a footprint-NAME table cannot encode when two parts share
a name (measured: C79924 vs C7719, both SOT-23-5, need 180° vs 90°). v1.3
regenerates the CPL through a new per-LCSC override table
(skills/jlcpcb-fab/scripts/jlc_lcsc_rotations.csv, per-LCSC WINS over the
name-DB), so every one of the 10 rows now ships its twin-measured exact-fit
rotation and the twin reports **ZERO** unresolved rotation suggestions. Full
before/after table + mechanism + twin proof: verification/rotation_fix_v1.3.md.
The v1.2 **gerbers, drills, and BOM are byte-identical here** (copper/schematic/
routing unchanged — a docs+CPL supersede); ONLY fab/cpl.csv and the
documentation/evidence changed. **v1.2 remains bare-PCB-orderable but is
HOLD-for-PCBA**: do not machine-assemble from the v1.2 CPL.

⚠️ **MANDATORY U1 pre-PCBA human gate — see §3a.** The exact-fit says U1 = 90°;
the machine cannot disambiguate a symmetric 180° package error by pad-fit alone.
Confirm U1's rotation against JLC's placement preview (package pin-1 dot / dot
corner vs the board pin-1 marker) BEFORE approving any assembly order.

**v1.2 SUPERSEDES v1.1** (second external review 2026-07-24, HOLD; archived
verification/external_review_v1.1_2nd.md + 08_reviews/): the XU316 0V9 core
rail carried **8 local 100 nF caps vs the vendor minimum of 12** —
XU316-1024-TQ128 datasheet XM-014532-PC-2.0.0 §14 "Integration" p.29: "Place
many (at least 12) 100 nF low inductance multi-layer ceramic capacitors close
to the chip" (verified against the datasheet, not taken on reviewer
authority). v1.2 ships **13× 100 nF (C_c1–C_c13) on 0V9**, the five new caps
anchored at the previously under-served core-VDD pins (worst pin-to-nearest-
cap now 3.22 mm; pins 50/54 improved 3.51/3.90 → 2.01/2.02 mm via the C_b0v9
bulk slot swap). Full measurement + v1.1-netlist diff:
verification/decoupling_fix.md. v1.1 is otherwise sound and all four of its
v1.0-blocker closures are intact and re-verified below. The same review's
other findings: rail-sequencing scope-gate strengthened (§4a, EXT2-F2);
D_USB stub + RJ45/OVP posture carried unchanged (EXT2-F3/F4, ADR-0007).

**v1.1 SUPERSEDED v1.0** (external review 2026-07-24, DO-NOT-ORDER; archived
...
Gates at seal (v1.3): copper unchanged from v1.2, so DRC/ERC/parity/audit are
carried on the byte-identical source/ (DRC 0/0/0 --severity-all --refill-zones
--schematic-parity, check_port_nets 115/115 labels + 8/8 ports, ERC 0 errors /
1211 baselined warnings, count_parity 199 x4, policy_audit 0 FAIL (R-LEN
GRADED: USB pair spread 0.110 mm), E-INV 7/7, audit_board incl. the U1-EP-16-
vias and USB skew/width/layer gates). RE-RUN against the corrected CPL: **twin
exit 0 (175 OK / 369 checked), ZERO unresolved rotation suggestions** (was 10 in
v1.2), C9900035627 adjudicated consign no-CAD; missing_models 177/177/0.
Freshness: fab gerbers/drills + bom.csv intentionally byte-identical to v1.2
(verification/freshness_exceptions.txt), fab/cpl.csv regenerated. Evidence in
`verification/`; hashes in `MANIFEST.txt`.
...
- **Assembly CPL:** `fab/cpl.csv` — **per-LCSC exact-fit rotations (v1.3 fix)**.
  10 rows were corrected off the v1.2 generic name-DB values to the twin's
  measured pad-fit: U1 270→90, U2/U3 270→90, U5 270→90, U7/U8 0→90, D_USB
  270→90, Q1/Q2 270→180, U9 270→180 (verification/rotation_fix_v1.3.md). Still
  eyeball the JLC preview per §3a — the DB is assembly-zero *truth* but the
  preview is the physical confirmation (MANDATORY for U1).
...
- 🔴 **MANDATORY U1 ROTATION GATE — BLOCKING before ANY PCBA order.** v1.3's
  CPL sets U1 (XU316, TQFP-128, consigned) to **90°** — the digital twin's
  exact pad-fit (fit 0.00 mm), which is the machine-best answer and a **180°
  correction** off the v1.2 CPL's 270°. A symmetric-package 180° error is
  INVISIBLE to a pad-fit, so this ONE part must be confirmed by a human against
  JLC's placement preview: **the package's pin-1 dot / bevelled dot-corner must
  sit at the board pin-1 marker (U1 pad 1, NW corner of the EP field at ≈(90,
  102) mm, top side).** If the preview shows the dot 180° opposite, STOP and
  escalate — do not approve. Archive the annotated preview screenshot for U1.
- Pin-1/polarity confirmation for **every** rotation-corrected row — now all
  report **OK** in verification/twin_report.txt with `src=lcsc`:
  **U1, U2, U3, U5, U7, U8, D_USB (90°), Q1, Q2, U9 (180°)**. The per-LCSC
  table values are assembly-zero *truth*; still verify each in the preview, do
  not blind-apply (per-reel deviations exist).
...
## 3b. ⚠ MSL-3 handling — consigned U1 (XU316, TQFP-128)

The XU316-1024-TQ128 is **moisture-sensitivity level MSL-3** (JEDEC J-STD-020;
0.4 mm-pitch TQFP with a large exposed pad — moisture trapped under the EP
flashes to steam at reflow and pops/delaminates the package). Because U1 is
**consigned** (supplied by us, not JLC's reel), moisture control is OUR
responsibility, not the assembler's. Before handing U1 to the line:
...
5. **Bake authorization:** if floor life is exceeded or the HIC failed,
   **authorize a bake** (typical 125 °C, duration per J-STD-033 for the body
   thickness) BEFORE reflow. Record the bake in the assembly-closure package.

Put these instructions in writing to the assembler with the consignment; an
...
per-port UNFUSED — §0). Budget the calibration sequence accordingly; the
durable fix is the F_BEEP PTC + spine SMBJ5.0A in the next rev (§6).
...
- **Rail-sequencing / reset scope-capture gate (EXT2-F2, STRENGTHENED in
  v1.2):** capture 3V3, 0V9, 1V8, 3V3A + RST_N (vs ADR-0005) across ALL of
  the following startup corners — a capture set is incomplete without every
  row:
  ...
  **PASS CONDITION (explicit): 1V8 must be VALID before the 0V9 core rail
  ...
  corner is untested at the others). The interlock is recorded as a v-next
  design item (§6).
...
- 0V9 core-rail ripple/droop at U1 during repeated boot, sustained USB
  High-Speed traffic, and maximum audio processing load (EXT2-F1 follow-up:
  the decoupling now meets the vendor minimum; the scope measurement is the
  empirical close-out).
...
- **Rail-sequencing interlock (EXT2-F2):** gate U8's EN from a 1V8
  supervisor or a combined rail-good signal instead of PG_3V3 alone, so the
  1V8-before-0V9 ordering is enforced by hardware rather than by RC-race
  topology. Mandatory if any §4a sequencing corner fails; recommended for
  production regardless.
- **In-line USB ESD (EXT2-F3, pre-production):** move TPD4EUSB30 directly
  in-line with D+/D− at J2, minimize connector-to-protector distance and
  ground inductance, then contact + air-discharge ESD testing while
  monitoring resets, USB disconnects, data corruption, and permanent damage.
  The current ~7 mm branch stub is bench-acceptable only.
