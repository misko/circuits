# ORDER README — crow-recorder-central-v2 (8-channel CENTRAL recorder) v1.7
...
Release **v1.7-2026-07-27**. Board **170.1 × 120.1 mm, 6 layer**.

## ✅ v1.7 SUPERSEDES v1.6 — ONE SOURCING SUBSTITUTION, NO COPPER CHANGE

**v1.6 is not wrong and its board is this board.** It is UNORDERABLE for one
reason: JLC cannot supply one of its 47 coded lines.

> **`C25767` (UNI-ROYAL 0402WGF2203TCE, 220 kΩ 0402, `R_vb1`) reads
> `stockCount: 0`.** It is ON the CPL — 1 placement × 5 boards = 5 needed — and
> unlike the other two stock-0 lines it carried no `sourcing_plan:` entry and
> was on no order-day watch-list. **At the v1.6 seal it measured 16 and was
> graded `OK`**, because nothing in the pipeline distinguishes "clears 5× need"
> from "clears 5× need by eleven units". Sixteen is inside a single day's churn
> of zero.

**The substitution, both records read live 2026-07-27 from JLC's own parts API
(`selectSmtComponentList`, exact `componentCode` match):**

| | `C25767` (out) | **`C138030` (in)** |
|---|---|---|
| MPN | 0402WGF2203TCE | **RC0402FR-07220KL** |
| brand | UNI-ROYAL (Uniroyal Elec) | YAGEO |
| package (`componentSpecificationEn`) | 0402 | **0402 — the same land** |
| library tier | expand | expand |
| MOQ (`leastPatchNumber`) | 20 | 20 |
| **stock** | **0** | **736 704** |
| `describe` | `-55℃~+155℃ 220kΩ 50V 62.5mW Thick Film Resistor ±1% ±100ppm/℃ 0402 Chip Resistor - Surface Mount ROHS` | **CHARACTER-IDENTICAL — compared as strings, not by eye** |

`R_vb1` is the top leg of the VBUS→VBUS_SENSE divider (220 k / 330 k into the
XU316 sense input). Same YAGEO RC0402FR series as the two swaps this board
already carries (`C60490` 10 kΩ, `C105871` 4.7 kΩ). **Q-IDENT (canon M-QUOTE):**
the arriving record's manufacturer MPN `RC0402FR-07220KL` EXACTLY equals the
authoritative MPN this repo already holds for that code in
`skills/jlcpcb-fab/references/lcsc_passives_ledger.yaml` — not merely equal
modulo a packaging suffix — so this is a SOURCED line, not a proposal.

**Changed at SOURCE, never in the CSV (canon M3).** One
`supplierPartNumbers={{ jlcpcb: ["C138030"] }}` in
`03_tscircuit/src/crow_recorder_central_v2.tsx`; `circuit.json` regenerated by
`tsci build`; the BOM row regenerated by `export_jlc_package.py`. A `bom.csv`
that moved without its `.tsx` moving would be a HAND-EDITED BOM, and the next
rebuild would silently restore the out-of-stock code.

**THE COPPER DID NOT MOVE, AND THAT CLAIM CARRIES NUMBERS**
(`verification/replot_identity.txt`):

| measured | result |
|---|---|
| `source/crow_recorder_central_v2.kicad_pcb` md5 | **`de39e145e856cb14d491770c77d1ec0a`** — identical to v1.6's, v1.5's and `04_kicad/`'s |
| gerbers + drills RE-PLOTTED from this release's own board | **17/17 byte-identical** to v1.6's sealed payload after stripping only the plot's own timestamps |
| `fab/cpl.csv` | **byte-identical** (174 rows) — a substitution moves no placement |
| `fab/bom.csv`, parsed as CSV rather than diffed as text | 49 → 49 rows, designator list identical **in order**, **exactly 2 cells changed**, both on `R_vb1` (`MPN`, `LCSC`), 0 Footprint changes, 0 Comment changes, 0 rows added or removed |
| `circuit.json` | 1 substantive delta (`R_vb1.supplier_part_numbers`); a CONTROL rebuild of the SAME input reproduces the warning-id churn, so that churn is the generator's and not this change |

**This is asserted by a gate, not by seven waivers.** v1.7 is gated with
`release_freshness_check.py --sourcing-supersede`, a mode that landed with this
release under canon M8 (two-strike promotion): usb-hub-3s-v3 v1.11 shipped the
identical shape on 2026-07-27 behind seven hand-written file waivers because no
mode permitted a changed LCSC. The mode ASSERTS the board md5, the byte-identical
CPL, the timestamp-stripped plot identity, the row count and designator ORDER,
that only `MPN`+`LCSC` moved and only together, that no MPN went blank, that
F-LEGIBLE still passes, that the `.tsx` changed, and that BOTH codes are named
in this document.

### 📋 THREE MESSAGES JLC WILL SHOW YOU THAT ARE EXPECTED, NOT ERRORS

Read this before uploading, so none of them reads as a defect:

1. **"designators don't exist in the CPL file"** — for exactly **13**
   designators: `J1`, `J3`–`J10`, `JP_INJ`, `J_DBG`, `R_inj1`, `R_inj2`. That is
   CORRECT and deliberate: those parts are on the BOM but are not placed by JLC
   (§3). `R_inj1`/`R_inj2` share their BOM line with `R_bg1`, which IS placed —
   JLC sources 3 and places 1. **Do not "fix" this at the order form.**
2. **`C9900035627` (RJHSE-5384, the 8 RJ45 jacks) unavailable / stock 0** — also
   correct. Not in JLC's assembly catalog, hand-soldered at integration, off the
   CPL. It is on the BOM only so the part is named in the order paperwork.
3. **`U5` resolved as `C131025` instead of our `C82317`** — see the F-ECHO
   section below. **The adjudication is ACCEPT.**

### 🔴 F-ECHO — the human gate, at upload time, NOT waivable

JLC RESOLVES our codes on their side and can silently redirect one. **After
uploading `fab/bom.csv`, save JLC's own resolved/matched part table out of their
UI and run:**

```
bom_legibility_check.py 07_releases/crow-recorder-central-v2-v1.7-2026-07-27/fab/bom.csv \
                        --echo <saved-jlc-table>.csv
```

Read the DENOMINATOR, not just the verdict: it prints `N/47 coded rows echoed`,
and passing on 3 of 47 rows is not a clean bill. The worklist is
`verification/bom_echo_gate.txt`.

**Expect exactly one finding, and its answer is ACCEPT.** Our source says
`C82317` for `U5` in three places; JLC's resolved output came back `C131025`.
Both were queried live 2026-07-27 and they are the SAME PART — MPN
`W25Q16JVSSIQ`, brand Winbond Elec, package SOIC-8-208mil, `describe` string
character-identical — with JLC's code carrying **7.5× the stock** (72 993 vs
9 765). This is a DUPLICATE CATALOG ENTRY consolidated toward the higher-stock
code, not a part change. It remains a FINDING by construction, which is the
point of the gate: **adjudicate it before paying, do not treat it as a defect.**
Anything OTHER than `U5` → verify MPN, brand, package and spec string against
`02_parts/` before accepting.

## ✅ v1.6 SUPERSEDED v1.5 — BOM LEGIBILITY ONLY, NO COPPER CHANGE
...
`verification/bom_echo_gate.txt`. (v1.7 restates this above with the U5
adjudication resolved: ACCEPT.)
...
- **Assembly BOM:** `fab/bom.csv` — `Comment,Designator,Footprint,MPN,LCSC`;
  49 lines, 47 coded, 2 deliberately uncoded (see §3). **It is NOT identical to
  any earlier release's**: v1.6 rewrote every row's `Comment`/`MPN` for
  legibility (md5 `c6a8ff14c563bd0b8913a5259bfff72d`, vs
  `b9e4486adb2813a7fe577638597a6f23` for v1.2/v1.3/v1.5), and v1.7 changed two
  more cells on `R_vb1`. This sentence claimed byte-identity through v1.6, where
  it was already false — **a docs fix that is not re-read at the next release
  becomes the next release's defect.**
...
Re-run a same-day stock check before paying — **and note that v1.6's
watch-list was the wrong list.** It named `C5224055` (383) and `C882626` (496)
and missed the line sitting at **16**, which is the line that went to 0. A
threshold that only asks "does it clear 5× need" cannot see a margin of eleven
units. Re-measured 2026-07-27 on THIS release's final BOM, all 47 coded rows
at build quantity 5: **45/47 clear**, and the 2 that do not are both covered —
`C6938291` (the XU316) at stock 0, which is exactly why it is CONSIGNED and why
JLC stock is irrelevant for it (§2, plus the `sourcing_plan:` entry in
`03_src/rules/assembly.yaml`), and `C9900035627` (the RJ45 jacks) at 0, which is
off the CPL entirely.

**A STOCK PASS IS NECESSARY AND NOT SUFFICIENT**, and the tool now says so in
its own output: *"graded against LCSC CATALOG stock (stockCount). JLC's assembly
uploader allocates from a DIFFERENT pool, so a PASS here does NOT mean the line
will clear at order time. A FAIL is real; a PASS is necessary and not
sufficient. The only instrument that answers 'will it clear' is the uploader
itself — F-ECHO."* Measured proof from this fleet: usb-hub v1.10 recorded
`"status": "OK", "stock": 291` for a line JLC then refused.

Thinnest coded rows on this release, worth an order-day glance in this order:
`C6035451` (U10) 1268 · `C232798` (U4) 1508 · `C181312` (U2/U3) 1575 ·
`C3020560` (J2) 3481 · `C5224055` (U7/U8) 383 for a need of 10 · `C882626` (L1)
496 for a need of 5. Everything else is ≥ 8 000, including the new `C138030` at
**735 674**.
...
| **R_vb1** | **220 kΩ 1 % 0402 (C138030, YAGEO RC0402FR-07220KL)** | **NEW at v1.7** — `C25767` went to stock 0 between the v1.6 seal and order day; `describe` string character-identical, same 0402 land, same RC0402FR series as the two rows above |
...
- The exact U1 consignment MPN + lot; final supplier + MPN for every
  manually-sourced line. **`fab/bom.csv` carries a resolved MPN on all 47 coded
  rows** — that is the entire reason v1.6 exists, and reading it off the BOM is
  now the shortest correct route. (Through v1.6 this line still said the MPN
  column was blank on all 49 rows: true at v1.4, false from v1.6 on, and it told
  its operator that the defect v1.6 fixed was still present.) The 2 uncoded rows
  are the `dnp_by_design` straps `JP_INJ` and `J_DBG`, which carry their
  footprint name instead.
...
- RG1/R_cs/R_rst/R_scl/R_sda **and now R_vb1** are EXTENDED parts (basic
  equivalents stocked out) — each extended reel adds a setup fee; if
  C25744/C25900/C25767 restock by order day, swapping back at source is a legal
  change for a FUTURE release (gate it with
  `release_freshness_check.py --sourcing-supersede`), **NOT an order-form
  substitution.** That rule is what forced this release rather than a
  substitution typed into JLC's order page.
