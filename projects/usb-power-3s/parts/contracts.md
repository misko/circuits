...
| Stock / price | seconds | hours | `build/cache/`, TTL'd, **never** here |
...
need a datasheet
  ...
  → extract facts ONCE into parts/<MPN>/part.yaml
  → is the part actually USED (in the schematic/BOM)?
      yes → copy the PDF from cache into parts/<MPN>/ and commit
      no  → cache-only. Record WHY it lost in docs/decisions/NNNN-*.md
```
...
- every part in the BOM has a `parts/<MPN>/` with a `part.yaml`
- every `parts/<MPN>/` is in the BOM (catches stale entries after a swap:
...
- BOM part with no `parts/` entry → fetch, extract, commit. Do not order
  until it exists.
- `parts/` entry not in the BOM → the part was swapped; delete the directory
  (git history keeps it) and note the swap in `docs/CHANGELOG.md`.
