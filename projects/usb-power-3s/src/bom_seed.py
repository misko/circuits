#!/usr/bin/env python3
"""Fill LCSC + MPN into build/fab/bom_jlc.csv from parts/<MPN>/part.yaml.
The map below is the explicit BOM-comment -> MPN assignment; every BOM line
must resolve (exit 1 otherwise), and every mapped MPN must exist in parts/."""
import csv, sys
from pathlib import Path
import yaml

HERE = Path(__file__).parent.parent
MAP = {  # BOM Comment -> parts/ MPN
...
bom = HERE / "build" / "fab" / "bom_jlc.csv"
...
for r in rows:
    ...
    if not mpn:
        ...
        continue
    y = HERE / "parts" / mpn / "part.yaml"
    if not y.exists():
        missing.append(f"{mpn}: no parts/ entry")
        continue
