#!/usr/bin/env python3
"""Generate kicad/usb_power_3s.kicad_pcb from the netlist + floorplan.
...
Run with KiCad-bundled python: /usr/bin/python3 src/generate_board.py
Then: python3 src/generate_rules.py  (netclasses+dru), and route.
"""
import re
import sys
...
K = HERE.parent / "kicad"
NETLIST = HERE.parent / "build" / "netlists" / "usb_power_3s.net"
...
def write_lib_table(libs):
    rows = ['(fp_lib_table', '  (version 7)',
            '  (lib (name "usb_power_3s")(type "KiCad")(uri "${KIPRJMOD}/../src/lib/usb_power_3s.pretty")(options "")(descr "project footprints"))']
