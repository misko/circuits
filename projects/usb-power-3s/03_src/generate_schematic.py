"""Generate 04_kicad/usb_power_3s.kicad_sch.
...
per 01_docs/ARCHITECTURE.md + 01_docs/DETAIL_DESIGN.md; decisions in 01_docs/decisions.
...
BODY = []
LABELS = []
NO_CONNECTS = []  # canon S4: sanctioned floats get EMITTED no_connect flags
...
def place(sym, ref, value, x, y, nets):
    ...
    for pin_num, net in nets.items():
        ...
        if net is None:
            # canon S4: a None net is a SANCTIONED float (documented in the
            # release's verification/pin_review.md) — emit the no_connect
            # flag at the pin's connect point so ERC enforces the sanction
            # instead of prose. 13 on this board: U2/U3 SYNCOUT+NC(9,15,16),
            # U4/U5/U6 FAULT (no MCU, resolution 2), J5 SBU1/SBU2 (power-only).
...
section("3. BUCK A  5.08V / 6A -> 5V_C (USB-C)   [math: 01_docs/DETAIL_DESIGN.md]", 175, 20)
...
sch.append('    (comment 1 "3S LiPo -> 3x USB-A 2.5A + USB-C 6A; 01_docs/ + ADRs in repo"))')
...
((HERE.parent / "04_kicad").mkdir(exist_ok=True) or (HERE.parent / "04_kicad" / "usb_power_3s.kicad_sch")).write_text(content)
...
# point the table at it. 96 warnings -> 0 (2026-07-17).
lib = ['(kicad_symbol_lib (version 20230121) (generator spf_generate_schematic)']
for s in SYMBOLS.values():
    lib.append(s.replace('(symbol "pwr:', '(symbol "', 1))
lib.append(')')
libtxt = "\n".join(lib)
assert libtxt.count("(") == libtxt.count(")")
(HERE / "lib" / "pwr.kicad_sym").write_text(libtxt)
(HERE.parent / "04_kicad" / "sym-lib-table").write_text(
    '(sym_lib_table\n  (version 7)\n'
    '  (lib (name "pwr")(type "KiCad")'
    '(uri "${KIPRJMOD}/../03_src/lib/pwr.kicad_sym")(options "")(descr '
    '"generated by 03_src/generate_schematic.py - do not hand-edit"))\n)\n')
...
if not (HERE.parent / "04_kicad" / "usb_power_3s.kicad_pro").exists():
    (HERE.parent / "04_kicad" / "usb_power_3s.kicad_pro").write_text(
...
print("wrote usb_power_3s.kicad_sch (+.kicad_pro) v3;",
      ...
      f"{len(NO_CONNECTS)} no_connect flags, parens balanced")
