...
## 2026-07-31 15:00 — re-staged from the rebuilt board, and the DELTA measured object by object

- did: **RE-STAGED `06_build/staging/` from the board that exists**, and
  measured the delta from the state the four 2026-07-31 lenses graded
  (`9af663f0`) rather than describing it. Control for every comparison is an
  ISOLATED WORKTREE at `9af663f0`
  (`<scratch>/wt_prefix`), so the pre-fix artifacts are regenerated by the
  SAME tools as the post-fix ones and no comparison rests on a remembered
  number.

### THE DELTA, CLASSIFIED — this is what scopes the fresh lens round

**`.kicad_sch` — IDENTICAL MODULO UUIDs. MEASURED, not assumed.** Masking
every UUID to a fixed token, the canonical sha256 of both files is
`2f3359965bf1e546…` — EQUAL. 476 UUID tokens each, 387 distinct each, and
**ZERO shared values**: the regeneration reassigned every one. Raw `diff` is
954 lines and **2 of them do not contain a UUID; both are the string `  )`**,
i.e. alignment artefacts, and the canonical compare proves it. **The
schematic/netlist lens's SOUND/ORDER verdict stands on the schematic and does
NOT need re-running.**

**netlist — 0 nodes moved, against real denominators.** `kicad-cli sch export
netlist` on both schematics: **40 nets / 130 nodes** each, net-name symdiff
**0**, node symdiff over shared nets **0**, netclass reassignments **0**.
Canonicalised (UUIDs + dates masked) the two `.net` files differ on **ONE
aligned line** — the `(source "…")` path of the input file. The graded parity
denominator is the narrower 24 nets / 114 nodes / 16 no-connects
(`kicad_sch_parity.py` excludes virtual `#PWR*`/`#FLG*`); both denominators are
now stated so neither reads as a discrepancy.

**`.kicad_pcb` — EIGHT vias and ONE design-setting value. Nothing else.**
Object-level diff keyed on geometry+net, UUIDs excluded:

| class | OLD | NEW | delta |
|---|---|---|---|
| footprints | 32 | 32 | **ALL 32 IDENTICAL** — lib_id, position, orientation, layer, value AND the full pad table |
| vias | 3446 | 3446 | **8 moved**, each paired to its predecessor: 0.0349 ×6, 0.0347 ×2. All GND, all 0.25/0.15 F.Cu↔B.Cu. 0 unpaired either way |
| track segments | 199 | 199 | IDENTICAL as a set with multiplicity |
| track arcs | 0 | 0 | — |
| zones | 6 | 6 | outlines, layers, nets IDENTICAL; **fill areas equal to 4 decimals on all six** |
| board graphics | 4 | 4 | IDENTICAL |
| board text | 51 | 51 | IDENTICAL |
| footprint text/fields | 88 | 88 | IDENTICAL |
| netclasses | 4 | 4 | IDENTICAL |
| nets | 40 | 40 | symdiff 0 |
| design settings | — | — | **`min_hole_to_hole` 0.25 → 0.315.** Every other setting equal |

The eight, old → new: `(28.6,46.2)→(28.631,46.216)`,
`(29.4,39.8)→(29.386,39.768)`, `(29.4,63.8)→(29.407,63.834)`,
`(43.8,36.6)→(43.816,36.631)`, `(50.2,60.6)→(50.232,60.614)`,
`(52.6,43.0)→(52.593,43.034)`, `(53.4,46.2)→(53.369,46.184)`,
`(59.8,28.6)→(59.792,28.634)`.

**gerbers/drills — 4 copper layers and the PTH drill. Nothing else.**
FIRST, A METHOD CORRECTION THAT MATTERS: **gerber bytes are NOT comparable
run-to-run.** Regenerating the PRE-FIX gerbers from the pre-fix board gave
13322 differing lines against the archive's own pre-fix gerbers — and
**order-insensitively the difference is exactly the two `CreationDate` lines**.
Flash emission order is nondeterministic in pcbnew. So every gerber comparison
below is over the command MULTISET. (This also PROVES the deleted archive's
gerbers really did describe the `9af663f0` board.)

| artifact | changed? |
|---|---|
| `F_Cu.gtl`, `In1_Cu.g1`, `In2_Cu.g2`, `B_Cu.gbl` | **CHANGED**, 32 commands each = the 8 via pads, flashed twice per layer, moved |
| `PTH.drl` | **CHANGED**, 16 lines = 8 hole coordinates moved |
| `F_Mask`, `B_Mask`, `F_Paste`, `B_Paste`, `F_Silkscreen`, `B_Silkscreen`, `Edge_Cuts`, `NPTH.drl` | **UNCHANGED** as command multisets — the vias are tented, so no mask/paste aperture moves |
| `bom.csv`, `cpl.csv`, `bom_echo_gate.txt`, `rotation_human_gate.txt` | **BYTE-IDENTICAL** |

**Does any change alter a fab deliverable's MEANING?** Yes, for exactly two:
the PTH drill program (8 holes) and the four copper layers (8 annular rings).
No, for mask, paste, silk, outline, NPTH and the whole assembly set.

### EVERY HEADLINE NUMBER THE FOUR LENSES RELIED ON

Re-measured with a script that shares no code with `fence_pitch.py`,
`fence_sites.py` or `audit_board` (canon M1), run identically on BOTH boards:

| number | OLD | NEW | verdict |
|---|---|---|---|
| fence worst interior gap / bound | 1.1769 / 1.1910 | **1.1769 / 1.1910** | UNCHANGED to 4 dp; 22 arm-sides, 0 OVER, `VERDICT: PASS`, RAW EXIT 0 |
| band-free nearest ground, max over all arms | 2.2142 (ANT3) | **2.2142 (ANT3)** | UNCHANGED. NOTE: the RF lens's own §5.4 states **2.2152**; my reimplementation gets 2.2142 on BOTH boards, so the 0.0010 is a sampling-convention difference between two reimplementations, not a board change |
| In1.Cu | 3437.1569 mm² = 93.9 % of 3662.3 mm², 0 track segments | **identical** | UNCHANGED |
| RF_ARMS copper spread | 0.1657 mm | **0.1657 mm** | UNCHANGED (R-LEN PASS, 8/8 measured) |
| via census | 3446, one class 0.25/0.15 | **identical** | UNCHANGED. GND vias 3433, PTH GND posts 40, fence elements 3473 — all UNCHANGED |
| ERC | claimed 0 / 209 | **0 / 213** | **MOVED — BUT NOT BY THIS FIX.** 213 = 124 `endpoint_off_grid` + 89 `lib_symbol_issues`. The PRE-FIX schematic measures **213 today too**, by both `.rpt` and `.json`, identical flags. 209 is not a property of either board; the fresh lens should measure, not inherit |
| BOM / CPL | 11 rows; top 27 / bottom 0 | **identical** | UNCHANGED, and the files are byte-identical |
| DRC | 0/0/0 | **0/0/0** | UNCHANGED. CLASSIFIED, both halves: violation types NONE, unconnected types NONE, parity NONE |
| min hole-to-hole (the point of the fix) | 0.3016 nom / 0.2366 max-mat | **0.3265 / 0.2615** | MOVED, as intended. Sub-floor pairs at max material **8 → 0** |
| **seed_stub barrels at d = 0.0000** | 28 vias declared, **ALL at d = 0.0000 exactly** | 36 declared, **28 at 0.0000 and 8 at 0.0002–0.0006 mm** | **MOVED — and the RF lens's §6 wording no longer holds.** |

**THE SEED-STUB FINDING, because a lens will hit it.** The RF lens wrote *"All
23 are present as real 0.2500/0.1500 mm GND vias at d = 0.0000 mm from the
declared coordinate. Not 'within tolerance' — exact."* The fix added 8 new
`seed_stubs` entries declared to FOUR decimals (`29.4065, 63.8344`), and the
emitter snaps to the MICRON, so the copper lands at `29.407, 63.834` —
**d = 0.00064 mm worst**. Physically nothing: 0.6 µm against a 125 µm via
radius, and the intended 0.035 mm displacement is delivered to 0.0347–0.0349.
But **the claim as WORDED is now false**, and the route.yaml comments beside
those eight entries predict hole gaps 0.0002–0.0003 mm optimistic against what
the board measures (e.g. declared 0.3366/0.2716, measured 0.3363/0.2713). The
count also changes denominator: the lens counted 23 ENTRIES (6 pin-serving +
17 fence); by VIA the same file is 28 → 36 (11 pin-serving + 25 fence).

### THE ARCHIVE

- `06_build/staging/` re-staged from the producers. `fab/bom.csv` and
  `fab/cpl.csv` come from `export_jlc_package.py` at `628ee3d4` — **nothing was
  hand-copied, renamed or hand-fixed on the way to any gate** (M-HANDFIX).
- **The archive stands alone: `source/` copied OUTSIDE the repo (`git
  rev-parse` confirms no repository there), `kicad-cli pcb drc --severity-all
  --refill-zones --schematic-parity` = 0/0/0, RAW EXIT 0, unpiped.** That run
  regenerated a `.kicad_prl` IN THE COPY — the mechanism the strip-last
  ordering exists for, observed rather than argued.
- **MANIFEST ↔ tree is BIJECTIVE IN BOTH DIRECTIONS**, by my own walker: 68
  rows, 68 files, 0 rows without a file, 0 files without a row, 0 sha
  mismatches. `sha256sum -c` agrees on the direction it can see (RAW EXIT 0,
  68 rows) — and a planted unlisted file is caught by the walker and invisible
  to `sha256sum -c`, which is why the walker exists.
- **A-EVID FAIL = 4, and the four are EXACTLY the review documents**
  (`redteam_layout.md`, `redteam_topology.md`, `pin_review.md`,
  `render_review.md`). 29 present. **This is the correct state and must not be
  closed by copying the pre-fix reviews in.** The previous revision's argument
  — "an absent required artifact is worse than a stale one" — is WITHDRAWN in
  §1 of the new ORDER_README: a stale review at a contract-required path is
  read by every name-based check as THIS release's verdict.
- `policy_audit` FAIL=1 (A-POP MANIFEST-UNDECLARED, the artefact of grading
  the PROJECT because `07_releases/` is empty), HUMAN=6, N-A=7, PASS=31, RAW
  EXIT 1. **S-OCCL is now PASS** — it was the second FAIL last revision.
  A-POP against THIS ARCHIVE is PASS, RAW EXIT 0.

### TWO THINGS I GOT WRONG AND CAUGHT, RECORDED BECAUSE THE MECHANISM REPEATS

1. **`jlc_twin` reported 11 CRITICAL and I nearly wrote it up as a delta.** It
   is not: `03_src/rules/twin_adjudications.yaml` is NOT auto-discovered, and I
   had omitted `--adjudications`. Proven by running the gate on the PRE-FIX
   board with the same omission — **identical 11 CRITICAL**. With the file:
   26 OK / 56 rows, bodies 27/27, RAW EXIT 0, matching the archive.
   **A gate whose verdict depends on a flag the caller must remember will
   eventually be run without it.**
2. **A `git check-ignore` sweep over `06_build/staging/` DELETED THE ENTIRE
   ARCHIVE — 69 files.** `06_build/*` is itself gitignored, so every staged
   artifact matched the blanket build-directory rule and read as
   contamination. Nothing was lost (every producer survived; the tree was
   rebuilt from them) but the check was WRONG, not merely unlucky. **The fixed
   rule maps each staged path to the location the archive will actually
   occupy — `07_releases/<version>-<date>/…` — and asks `git check-ignore -v`
   THERE**, so only the junk rules can fire and git's own matcher stays the
   authority. It carries a KNOWN-BAD FIXTURE: a planted `.kicad_prl` at the
   seal path IS caught, so the strip can fail. This is the M-NEGATION shape —
   a matcher that cannot tell "ignored because it is junk" from "ignored
   because it lives in the build directory".

- next: **A FRESH SCOPED LENS ROUND against `06_build/staging/`.** The delta
  above scopes it: the SCHEMATIC and the NETLIST did not move (canonical
  sha256 equal, node symdiff 0), so the schematic/netlist lens's SOUND/ORDER
  verdict does not need re-deriving; the RF lens's fence, band-free, In1.Cu,
  arm-spread and via-census numbers are UNCHANGED and only its §6 seed-stub
  exactness claim moved; the fab lens's own finding is the one this work
  closed; the release-integrity lens is the one with genuinely new material
  (new MANIFEST, new ORDER_README, A-EVID FAIL=4 by design, and the
  reproducibility caveat below). **NOT SEALED, and two blockers stand: A-EVID
  FAIL until the fresh reviews land, and `git_dirty: true`.**
- next: **THE ARCHIVE IS NOT BYTE-REPRODUCIBLE FROM COMMITTED STATE, and it is
  not this board's doing.** `git status projects/pluto-rx2-8way-v2/` is EMPTY.
  But four SHARED generic-backend scripts this board builds through are
  uncommitted — `generate_board_generic.py`, `generate_rules_generic.py`,
  `pcb_toolkit.py`, `route_and_stitch_generic.py` — and MEASURED by mtime,
  THREE of the four changed AFTER the board file was written (12:33), one of
  them at 14:44 while this archive was being staged. `rebuild_all.sh` today is
  not the driver that produced `source/*.kicad_pcb`. Recorded in `MANIFEST.txt`
  and ORDER_README §7 item 2 rather than stamped away; `git_dirty: false` is a
  seal precondition.
- next: **the two owed CLAIM corrections travelled into the archive** as
  ORDER_README §8. (a) The tap's "whole 440 Ω lumped at the junction,
  RL ≥ 25.5 dB" is overstated **1.61×**: only `R_T1` = 220 Ω is at the
  junction, `R_T2` sits 1.180 mm / 15.10° at 6 GHz downstream, and the measured
  floor is **273.85 Ω / RL 21.45 dB**. Still SOUND — the defect it replaced was
  5.80 Ω / RL 1.71 dB, so the fix moved the worst case **47×** — but the CLAIM
  is what a reviewer checks against, and `floorplan.yaml`'s intent block owes
  the correction at the next revision. (b) Same root cause makes the silk's
  −20.26 dB about 1 dB optimistic at 6 GHz (−21.6399 as built); a QUALIFIER,
  not a number change, since silk cannot move without a new release.
- next: **the vendor question is recorded as a DFM item, not as solved.** The
  tight class is a VIA against a PTH PAD; JLC publishes via-to-via and
  pad-to-pad floors and NOTHING for the mixed pair (their Q&A #693 asks exactly
  this, unanswered). `MANIFEST.txt` `fab:` and ORDER_README §7 item 4 both
  carry it with the exact question to put to JLC **in writing before ordering**.
