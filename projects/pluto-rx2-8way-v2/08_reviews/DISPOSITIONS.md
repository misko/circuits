...
| **`ORDER_README` carries `design_verdict: PENDING`, outside the closed vocabulary, and claims a 62 ↔ 62 bijection where the truth is 63** (2026-07-31, archive-integrity) | **FIXED.** Verdicts are `DEFECTIVE` / `DO-NOT-ORDER`, inside `SOUND\|DEFECTIVE` and `ORDER\|DO-NOT-ORDER\|BLOCKED-SOURCING`. The bijection count is no longer retyped into prose at all — the MANIFEST footer states it (81 files) and the check is RUN rather than quoted: 81 rows ↔ 81 files, 0 rows without a file, 0 files without a row, 0 hash or size mismatches on an independent re-hash of all 81. |
...
| **`release_freshness_check`: DESIGN FAIL, 11 findings** | **OPEN, and ALL ELEVEN ARE IN REVIEW DOCUMENTS — zero about the board, the fab set, or any machine evidence.** 9 × `EVIDENCE PATH MISMATCH` — review files naming the `v1.0-2026-07-30` release directory they were run against, which was never created. **One of the nine is THIS FILE**, and the recursion is worth naming: quoting the offending path inside the ledger that reports it makes the ledger match the gate's own regex, so a document describing the finding BECOMES one. The quotes above are written without the directory prefix for exactly that reason — not to dodge the gate, but because a ledger row is a description and not a citation of a path anyone should follow + 2 × `REVIEW-NO-VERDICT` on `redteam_topology.md` / `redteam_layout.md`, which DO state `design_verdict: DEFECTIVE` — at lines 211-212 and 77-78, below the 40-line header window the gate reads. **Neither may be closed by editing a review** (this contract makes review files append-only, verbatim) **nor by choosing a seal name that matches the stale path.** Both close when the fresh scoped red-team writes reviews naming the archive it actually graded, and those become the promoted lens files. |
| **`M-REL` is UNGRADED on this archive** | **OPEN, and it is a GATE property, not a claim about these hashes.** M-REL's regex (`policy_audit.py:1643-1646`) admits two MANIFEST layouts and this archive ships a third (`<hash>  <size>  <path>`); its zero-coverage backstop at `:1660` tests for the literal substring `sha256:`, which this banner does not contain. So M-REL verifies ZERO of the 81 rows and a green M-REL here would be evidence of nothing. The hash table was written because it is correct. `policy_audit.py` is another workflow's file — reported, not edited. |
