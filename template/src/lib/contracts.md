...
- A footprint's polarity / pin-1 marker is a FACT other tooling depends on.
  ...
  corresponding `parts/<MPN>/part.yaml` (`pins:`) — generators trust that
  file over the footprint.

## Validate

- `fp-lib-table` uses `${KIPRJMOD}`, no absolute paths
- every footprint referenced by `kicad/<board>.kicad_pcb` resolves from
  `src/lib/` or a stock KiCad library — no dangling refs
