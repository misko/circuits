...
design. Everything in `kicad/` and `build/` is derived from here. If `src/`
and `kicad/` disagree, `src/` is right and `kicad/` is stale.
...
| File | What |
|---|---|
| `generate_schematic.py` | emits `kicad/<board>.kicad_sch` from a component/net table |
...
- **One-off experiment scripts.** `manhattan_route.py`, `import_ses.py`,
  ...
  not `src/`. Rule: every script in `src/` must be runnable TODAY against the
  current board and produce the current result.
- Writing to `releases/`. Generators may write `kicad/` and `build/` only.
- Board data hardcoded where it duplicates `parts/` or `rules/` — see below.
...
4. **Pin numbers are PHYSICAL PADS, from `parts/<MPN>/part.yaml`** — not
...
- no script writes to `releases/`
- pin maps in the generator match `parts/<MPN>/part.yaml` (no divergence)
