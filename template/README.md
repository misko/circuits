...
contract is checkable. An agent entering `02_parts/` can read one file and know
...
```
01_docs/        human truth: architecture, math, decisions, changelog   [hand]
03_src/         THE source: generators, rules/nets.yaml, libs           [hand]
02_parts/       per-MPN datasheet + extracted facts                     [hand, once]
04_kicad/       the KiCad project — GENERATED, committed for diffs      [generated]
05_firmware/    code that runs on the board                             [hand]
06_build/       anything regenerable — gitignored, rm -rf safe          [tools]
07_releases/    one IMMUTABLE dir per fab order, with a MANIFEST        [write once]
```
...
3. **Intent and enforcement come from one file.** `03_src/rules/nets.yaml` says
