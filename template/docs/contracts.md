...
| File | What | Rule |
|---|---|---|
| `ARCHITECTURE.md` | the high-level concepts: power tree, net domains, stackup, ground strategy, critical geometries | prose + diagrams; the "why". Machine-readable net facts belong in `src/rules/nets.yaml`, not here — link to it |
...
- Part datasheets or PDFs → `parts/<MPN>/`.
- Stock, price, availability → volatile; `build/cache/`, never committed as truth.
- Generated renders → `build/renders/`. Committed images are allowed ONLY if
  hand-drawn (block diagrams) and their source is in `src/`.
...
- `## Power tree` — every rail: source → conversion → load, with current.
  Name the nets exactly as they appear in `src/rules/nets.yaml`.
...
`Released:` is `no`, or the `releases/` dir name. It is the only link between
a revision and a fab order.

## Validate

- every net named in `ARCHITECTURE.md` exists in `src/rules/nets.yaml`
- every `CHANGELOG.md` entry has a git tag that exists
- every `Released:` value that is not `no` names an existing `releases/` dir
- no `decisions/` content duplicated into `ARCHITECTURE.md`
- `DETAIL_DESIGN.md` numbers match the values in `src/` (spot-check on review)
