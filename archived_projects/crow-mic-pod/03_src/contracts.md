...
| `route_waves.sh` | **STOPGAP (canon M8)**: KRT re-route-from-scratch wave driver (hardest-first order); the standard `rebuild_all.sh` imports the PROMOTED `route/*.kicad_pcb` chain and does NOT call this. Backend gap: a config-driven route-wave schema in the shared `route_and_stitch_generic.py` would replace it | manual re-route only |
