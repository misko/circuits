...
| `prepare_placement_review.sh` | **PROJECT-LOCAL STOPGAP for the missing shared review-commission adapter.** After route prep it first proves promoted-route compatibility, then prepares content-addressed top/isometric native renders, an `INCOMPLETE` A-RENDER request, and a JSON commission under `06_build/pre_route/placement_review_requests/<subject-id>/`. The subject ID binds the exact track-free board/parts/rules and normalized prepared copper/placement DRC; raw r0 remains informational because pcbnew churns UUID/order bytes, and raw DRC remains informational because KiCad writes an export date. It never edits canonical human witnesses, never grants acceptance, publishes a complete request atomically, and reuses an exact immutable request on semantic reruns. A second board needing this workflow triggers promotion to the shared backend. | both drivers through bounded `pcb_flow.py` stage `placement_review_prepare` |
...
| 6a | `prepare_placement_review.sh` inside bounded `pcb_flow.py` stage `placement_review_prepare` — when the exact canonical reviews are already current, preserve them and atomically write a truthful `ALREADY_ADMISSIBLE` boundary pointer with no commission paths. Otherwise prove P-ROUTEBASE before review spend, render the exact track-free board with independent 90 s child deadlines, and atomically publish an immutable content-addressed commission whose status and A-RENDER request are explicitly `INCOMPLETE`. The canonical pin/layout/render/A-RENDER paths are named as required human actions but never opened for writing. | bash + `kicad-cli` + `/usr/bin/python3` |
| 6b | `$S/pre_route_review_check.py . --phase placement --board 04_kicad/<board>.kicad_pcb` — **P-ROUTEBASE + PR-REVIEW** remains the sole authority: first prove an existing promoted route derives from this exact prepared r0 (placement, source/prepared vias and prepared segments), then require independent pin/layout/render reviews plus same-camera A-RENDER bound to the exact track-free board and adopted design-rule SHA before route import. On stale evidence, the immediately preceding commission makes the stop deliberate and actionable; no preparation output can satisfy this gate. | `/usr/bin/python3` + human/fresh-context review |
...
**No placement-resume flag.** Measured on the exact v4 subject, route prep took
0.51 s, first native evidence preparation took 19.77--21.15 s, and an unchanged
semantic rerun reused the immutable request in 1.06--1.07 s. A second resume
checkpoint would therefore add mutable state without avoiding material work;
the existing schematic resume remains the only justified resume boundary.
