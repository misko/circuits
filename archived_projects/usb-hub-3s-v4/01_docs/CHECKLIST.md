...
- [ ] `03_src/audit_board.py` → PASS when present; otherwise the generic
      backend's shared placement gates in `03_src/rebuild_all.sh` all PASS
- [ ] `pad_separation.py 04_kicad/usb_hub_3s_v4.kicad_pcb --project .` → P-PADSEP
...
mergeable.
- [ ] release inputs clean (`git_dirty: false`, scope
      `projects/usb-hub-3s-v4/ + skills/` via
      `release_git_dirty.py usb-hub-3s-v4` — a dirty sibling board does not block)
- [ ] tagged
...
- [ ] `policy_audit.py projects/usb-hub-3s-v4` → zero FAIL; waivers evidence-backed; HUMAN items carry the fresh-context reviewers' verdicts
