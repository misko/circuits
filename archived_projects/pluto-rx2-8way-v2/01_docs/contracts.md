...
| `ORDER_README.md` | **the SOURCE of the order/assembly document** — every fab option, human gate, bring-up step and safety warning the release ships. Hand-written prose, exactly like `BRIEF.md` and `DETAIL_DESIGN.md`, and tracked for the same reason | the copy under `06_build/staging/` is BUILD OUTPUT (gitignored) and is derived from this one, ONE WAY. Added 2026-07-31 because it had **no tracked home until seal**: it lived only at `06_build/staging/ORDER_README.md` under a `06_build/*` ignore rule, so on any unsealed board it was invisible to git, absent on a fresh clone, and destroyable by a re-stage with no diff and no gate noticing. MEASURED: every SEALED board's copy is tracked (it becomes so only when the seal copies it into `07_releases/`), and `pluto-rx2-8way-v2` had exactly one copy and it was untracked — while carrying the 0 VDC abs-max RF-port warning. Same family as the 22 lost DISPOSITIONS findings: work with no durable home reads exactly like work that was never done. **OWED: the template at `skills/pcb-design/templates/contracts/01_docs/contracts.md` needs the same row** — this is fleet-wide, not board-local, and that file is held by another session |
