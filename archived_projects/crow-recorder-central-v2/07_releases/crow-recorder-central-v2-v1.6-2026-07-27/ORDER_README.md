# ORDER README — crow-recorder-central-v2 (8-channel CENTRAL recorder) v1.6
...
Release **v1.6-2026-07-27**. Board **170.1 × 120.1 mm, 6 layer**.

## ✅ v1.6 SUPERSEDES v1.5 — BOM LEGIBILITY ONLY, NO COPPER CHANGE

**v1.5 is not wrong and is not DO-NOT-ORDER.** Its board is this board: the
`.kicad_pcb` is md5-identical (`de39e145e856cb14d491770c77d1ec0a`) to v1.5's
and to `04_kicad/`'s, the gerbers and drills RE-PLOT from this release's own
source 17/17 byte-identical after the timestamp strip, and `fab/cpl.csv` is
byte-identical. Every review verdict v1.5 carries stands unaltered. **Only
`fab/bom.csv` changed.**

**What was wrong with it.** v1.5's BOM was uploaded to JLCPCB and the parts
"were not being picked up by their web processing". Graded the way the
RECIPIENT parses it (canon F-LEGIBLE, ADR-0006) the sealed file has **72
findings**:

| check | findings on v1.5 | what JLC saw |
|---|---|---|
| F-MPN | 47 | every coded row ships a **blank MPN**, so JLC's matcher leaves a code-only line at *No Part Selected* |
| F-WORDS | 24 | the Comment is an LCSC code or `simple_chip`/`simple_inductor` — a row no human on either side can review |
| F-ENCODE | 1 | `Ω` with **no UTF-8 byte-order-mark**, so a reader defaulting to cp936 renders `CE A9` as `惟` |

**This release: 0 findings.** 47/47 coded rows carry an MPN resolved from
`02_parts/<MPN>/part.yaml` or the vetted passives ledger, 49/49 Comments read
as values or part numbers, and the file carries the byte-order-mark. The two
uncoded `dnp_by_design` strap rows (JP_INJ, J_DBG) now read as their footprints
— `PinHeader_1x03_P2.54mm_Vertical` / `PinHeader_1x08_P2.54mm_Vertical` —
because their board Value is a tscircuit generator stand-in (`simple_chip`)
that cannot be corrected without regenerating the board, which would move
copper.

**Nothing about SOURCING changed.** Every row's designator group, footprint and
LCSC code is identical to v1.5's; only the Comment and MPN columns moved, and
`release_freshness_check.py --legible-bom-supersede` FAILs on any other kind of
delta — including a changed LCSC, which would be a substitution.

### 🔴 F-ECHO — the human gate this release adds (do it at upload time)

JLC RESOLVES our codes on their side and can silently redirect one: our source
says `C82317` for U5 in three places and JLC's resolved output came back
`C131025`. **After uploading `fab/bom.csv`, save JLC's own resolved/matched
part table out of their UI and run:**

```
bom_legibility_check.py 07_releases/crow-recorder-central-v2-v1.6-2026-07-27/fab/bom.csv \
                        --echo <saved-jlc-table>.csv
```

A code JLC redirects is a **SUBSTITUTION and a FINDING**, not a convenience —
adjudicate it before paying, never after. The 47-line worklist is
`verification/bom_echo_gate.txt`.

### The one other v1.6 change, and it is an INPUT not a payload file

`03_src/rules/power_tree.yaml` now DECLARES the board's two LDO rails, **1V8**
(U9) and **3V3A** (U10). It changes no gerber, no drill, no placement and no
BOM row — it is listed here only so a reader six months out is not left
wondering what else moved. **Nothing about the board changed; what changed is
how much of it a gate can see.**

Until 2026-07-27 canon E-TOPO could not express a linear regulator, so both
rails lived in a COMMENT in that file and **two of this board's three
regulators had never been graded by anything**. E-TOPO gained LINEAR support
that day and immediately reported `UNGRADED CONVERTERS: 2 of 3`. Sealing v1.6
on top of that would have shipped a WORSE gate profile than the release it
supersedes, so the rails are declared and now graded on the two failure modes
the topology derivation is blind to:

| rail | converter | headroom vs dropout | PD vs package rating |
|---|---|---|---|
...
Both dropout figures are the datasheet **MAX**, both quoted at a HIGHER current
than the rail draws (150 mA and 300 mA), so each bounds its rail from above.
Every number is cited to figure/page in the converter's own `part.yaml` — see
`verification/power_topology.txt` for the graded output.

E-TOPO on this board: **FAIL (2 of 3 converters ungraded) → OK, 4/4 rails
covering 3/3 converters.**
