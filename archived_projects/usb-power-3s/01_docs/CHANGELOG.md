# Changelog — usb-power-3s

## v1.3 — 2026-07-17
Released: 07_releases/v1.3-2026-07-17/
- GND pins draw standard power-symbol ground icons (82) instead of GND
  label plates — schwriter2 engine change: hidden power_in pin names the
  net, #PWR refs virtual, PWR_FLAG on J1.1 (XT60 '-') keeps ERC
  power-pin checks at 0. Rails stay labels by design.
- Netlist node-for-node identical (parity 0, 68 nets/303 nodes); ERC 0
  errors (isolated_pin_label PGOOD_B baseline unchanged); policy_audit
  zero FAIL; S-OCCL 0 (icons are not text).
- Fab byte-identical to v1.1-v1.2 (ADVANCED small-via requirement
  carried forward unchanged). Supersedes v1.2.

## v1.2 — 2026-07-17
Released: 07_releases/v1.2-2026-07-17/
- Schematic generated by the schwriter2 layout engine (kicad-pcb skill):
  structure-only declarations, all coordinates from text envelopes, 25
  real wires (gate networks, FB dividers, ILIM chains, UVLO/OV ladder,
  PGOOD sequencing, input protection story), collision-free by
  construction (internal S-OCCL assert + independent check both 0;
  waiver removed, 65 -> 0). Netlist node-for-node identical (parity 0,
  68 nets/303 nodes); ERC 0 errors (96 off-grid warnings eliminated);
  board untouched.
- Fab byte-identical to v1.1 — the ADVANCED small-via option (0.25/0.15
  fanout vias) remains REQUIRED. Supersedes v1.1.

## v1.1 — 2026-07-16
- CRITICAL: vendored VQFN-20 LM5145 footprint was MIRROR-NUMBERED (CW winding
  vs datasheet CCW) — found by the new jlc_twin stage comparing our footprints
  against JLCPCB's own CAD; confirmed by datasheet fig 6-1 + KiCad TI lib.
  Pads renumbered (n -> 21-n), full KRT re-route (taps hardest-first in-chain),
  gate back to DRC 0/0/0. v1.0 is SUPERSEDED — DO NOT ORDER v1.0.
- New order gate: JLC digital twin (footprint correspondence, rotation audit,
  adjudication register, twin renders); 74 fits verified <=0.4mm, 4 finding
  classes adjudicated with evidence.
Released: yes — 07_releases/v1.1-2026-07-16/

## v1.0 — 2026-07-16
- Full design from scratch: architecture + math docs, 5 ADRs, 38-line BOM
  (41 02_parts/ entries), generated schematic (96 components, 68 nets),
  100×60 mm 4-layer board, KRT-routed + scripted power stitching.
- Gate: DRC 0 violations / 0 unconnected / 0 parity issues
  (`--severity-all --refill-zones --schematic-parity`), placement audit PASS,
  polarity audit PASS, JLC stock check PASS (37 coded lines in stock).
- Fixed during bring-up: D2/D3 rail TVS were mapped to a SOT-23-6 ESD-array
  footprint (copy-paste from a previous project) — now D_SMB.
Released: yes — 07_releases/v1.0-2026-07-16/
