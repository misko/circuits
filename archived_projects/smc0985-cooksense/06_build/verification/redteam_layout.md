...
- Stale v1.0/v1.1/v1.2 artifacts in verification/ (renders showing J_CONTACTOR
  ...
  einv_red_verification proving 3 of 63 on a release claiming 83).
  CORRECTION 2026-07-26: this line originally claimed all were regenerated. They
  were NOT — the first re-stage regenerated the renders and stock_check but
  carried semantic_battery.txt, einv_red_verification.md, pin_review.md,
  render_review.md and parity.md forward unchanged. The frozen-archive lens
  caught that. All are regenerated NOW, and an INDEX.md names which verification
  files are current so the question is answerable without re-reading each one.
...
- **CPL rotations**: all 189 reproduced from `(board_rot + measured per-LCSC
  ...
  180 out.
- **Vias**: all **1047** at 0.25 mm / 0.15 mm — the ADVANCED small-via option is
