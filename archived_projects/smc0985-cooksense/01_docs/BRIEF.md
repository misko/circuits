...
| D12 | **The declared operating ambient is 65 °C, and a SIX-MEASUREMENT BENCH GATE is MANDATORY before the board is trusted above bench conditions** (user ruling 2026-07-30, v1.7). This narrows the envelope from the verbatim brief's enclosure **HARD** rung (75 °C) to its own **`stop`** rung (65 °C); 75 °C is RETAINED as the SURVIVE corner. Reason: the board is **thermally** limited, not dropout-limited — the dropout margin moves only **−0.31 mV/K** (copper is 42 % of the series sum), so narrowing the ambient buys it **+1.4 mV**, while the junction moves 1 °C per °C. At 65 °C the junction margin is **17.92 °C citable / 13.3–16.4 °C carrying the board's other 0.958 W**, against **7.92 / 3.3–6.4 °C** at 75 °C; dropout is **+19.5 mV** worst case with the CITED 2 × 10 mΩ Micro-Fit contact maximum. **The 7.92 °C figure the release used to publish was wrong regardless of the envelope** — the form omits a term worth 20–59 % of itself. The bench gate (ORDER_README §7b, B1–B6) retires `F1` R-vs-T, the Micro-Fit contact resistance in THIS build, **the AMS1117 dropout at 0.2 A** (today a 0.8 A datasheet figure applied to a 0.2 A rail — it dominates), `θ_JA` on this mounting, `ΔT_board`, and the SOT-223 thermal time constant; **passing B1–B6 may reopen 75 °C** as a docs-only supersede. Options considered and NOT chosen: tab-vias (recommended for the next copper revision — −18.0 °C/W, +8.4 °C, **no BOM change**), a switching regulator (a v2 decision), and sealing at 75 °C on the bench gate alone. ADR-0029, `power_tree.yaml`, ORDER_README §0-T/§7b. | narrows §3 "enclosure <=50 / 55 / 65 / 75"; ADR-0026/0027/0028 → ADR-0029 |

### Commission fact-lock — cooksense (Board A+B, main; v1.7 2026-07-28; thermal envelope 2026-07-30)

| row | value | basis |
|---|---|---|
| **thermal envelope** | **DECLARED OPERATING AMBIENT `Ta` = 65 °C. SURVIVE corner 75 °C.** Junction margin at the declared ambient: **17.92 °C** citable (`Tj = 65 + 42.0795 = 107.0795 °C` against ds1117 `Tj_max` 125 °C, `θ_JA` 90 °C/W SOT-223 package figure, `PD` 409.8 mW pass-element + 57.75 mW quiescent) / **13.3–16.4 °C** carrying the board's other 0.958 W (+1.55…+4.65 °C, DERIVED). Citable ambient ceiling **82.92 °C**, honest **78.3–81.4 °C**. `pdiss_max_mw` is HELD at the 75 °C derating (497 mW) on purpose — narrowing a declaration must not relax a gate. **MANDATORY bench gate B1–B6 before use above bench conditions.** | D12 / ADR-0029 / `power_tree.yaml` / ORDER_README §0-T, §7b |
| **input envelope** | **`J_PWR` 4.850 – 5.250 V DC SELV, ≥2 A (pref 3 A), keyed 2-pin Micro-Fit, measured AT THE CONNECTOR UNDER FULL LOAD.** This is the specification, not a mitigation. Derived node envelopes (**RE-DERIVED 2026-07-30, ADR-0027/ADR-0028; the old row said `4.754–5.25 V (minus 95.2 mV of worst-case F1 + Q_REV + eFuse at 0.50 A)` and omitted BOTH the board's own copper and the connector contacts**): `5V_PROTECTED` **4.750**–5.25 V at `U_EFUSE.5`, and the rail is NOT one node — its members span 48 mV of track, so `U_LDO.3` sits at 4.728 and `Q_COIL.2`/`R_STOPRAIL.1` at 4.702. The series sum is **328.29 mΩ** (137.79 mΩ of board copper + 190.5 mΩ of F1/Q_REV/eFuse) at a MEASURED 0.4024 A, not 95.2 mΩ at a guessed 0.50 A. **STILL NOT IN THAT SUM: 20–60 mΩ of Micro-Fit contact resistance**, which is what separates the node this row specifies (AT THE CONNECTOR) from the node the sum starts at (the PCB pad) — 8.1–24.1 mV against a +29.1 mV margin. See ADR-0028. | D11 / ADR-0021 / `power_tree.yaml` |
| output rails (V range + Imax) | `3V3` 3.201–3.399 V @ **0.20 A** (AMS1117-3.3, ds1117 p.2 full-temperature bold limits; **0.30 A was never derived — ADR-0026 replaced it with an itemised 116.859 mA × a 1.5 design margin**); `5V_PROTECTED` **4.750**–5.25 V @ 2.0 A budget; `5V_KEY_RELAY` @ 0.15 A; `5V_STOP` @ 0.02 A; `3V3_ANALOG` + four switched `3V3_SW_*` @ 0.05/0.10 A. | `power_tree.yaml` |
...
| hard-cell sourcing class | **`DIP05-1A72-13L` ×12** + PCC-SMP-K: **self-supplied, hand-solder, DO-NOT-SUBSTITUTE**. ⚠️ **CORRECTED 2026-07-30 (was "DIP05-1A72-12L ×13" — wrong code AND wrong count; found by the v1.7 topology red-team lens as P1-7).** `-12L` is a **DIFFERENT PIN-OUT** (code 12 = eight leads, 1↔14 tied as one contact node, 7↔8 the other, coil on the inner pins); it is the exact land defect that makes **all six sealed releases v1.0–v1.6 DO-NOT-ORDER**, and fitting it here shorts `5V_KEY_RELAY` to `U_SEL_BUS` and every ULN2803 output to its keypad line. **DO NOT re-map a code-12 part onto this land**, and note that **every distributor quote recorded in this tree (Mouser 876-DIP05-1A72-12L, DigiKey DIP05-1A72-12L-ND) is keyed to `-12L` and DOES NOT TRANSFER** — no `-13L` distributor stock figure has been read yet, so **distributor sourcing for the correct code is still OWED** (LCSC `C1524853` is the one catalog hit). `R_OVT`/`R_OVB` are **code-pinned protection setpoints** (C270658 / C407739, ±0.5%) — an auto-picked passive code must never set a cutoff threshold. | 02_parts/DIP05-1A72-13L, ADR-0021, `fab/bom.csv` |
