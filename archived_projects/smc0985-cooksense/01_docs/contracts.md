...
| `gh08-clone-mate-confirm.md` | the USER-HELD physical-part gate instrument for the `C265111` → `C42376901` (SHOU HAN GH-08) sourcing substitution: what to order with LCSC codes, the GH1–GH7 measurement table with a PASS/FAIL threshold and a provenance for each, and the report-back block. Also stages the v1.8 cut that a PASS unblocks | project-specific, same shape as `10fdz-bt-land-pattern-confirm.md`. Records only what a human must measure on a real part plus the evidence already in hand so the bench does not re-derive it; carries no CAD truth the sealed board does not already hold, and the staged `part.yaml` inside it is a fenced block, **not** a `02_parts/` directory — that would be a stale entry under `02_parts/contracts.md` until the part is on the board. Closed by appending a dated `§6 RESULTS` and, on PASS, cutting v1.8 |
