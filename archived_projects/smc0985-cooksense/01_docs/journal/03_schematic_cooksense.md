...
## 2026-07-24 13:55 — iterate (schematic gate GREEN; commit provenance note)
- did: ran the schematic chain. tsx_preflight PASS; gen_tscircuit ERC 0 errors (1154 baselined warnings); circuit.json == manifest (215 components); netlist re-exported; E-INV 60/60 GREEN. RED-TESTED the 45 new invariants against the SEALED v1.1 netlist in a scratch tree: they FAIL there on exactly the defect wiring (K_STOP.1 on 5V_KEY_RELAY, U_ONESHOT.3 on bare DOOR_OK, U_SR1.12 on unfrozen KEY_LATCH, U_FAULTAND.6 tied 3V3, all new pulls absent) — the checker can block. Also fixed: unquoted `adr: 0010/0011` in YAML parse as OCTAL (8/9); quoted.
