...
1. **`rails[].iout_max_A` = 0.20 A**, an ITEMISED SUM at datasheet maxima:
   on-board logic 4.804 mA + `3V3_ANALOG` 4.214 mA + off-board 9.572 mA + the
   four switched sensor rails 85.116 mA = **103.706 mA**, × a declared 1.5
   design margin for the terms that are not individually citable (CMOS dynamic
   current, the LVC ΔICC adder, MLCC leakage, harness leakage) = 155.6 mA.
   **Declared 0.200, not 0.160, because the count is DISPUTED and the dispute
   is recorded rather than averaged away:** the v1.7 re-gate-2 topology lens
   recounted this rail independently and reached **115.5 mA** against my
   103.706, naming four loads it says are absent here. Two of its findings are
   reproduced and were mine to get wrong — `R_ESTOPPD` is **470 Ω** on the
   staged BOM, not the 100 kΩ an earlier draft of this ADR assumed (a **213×**
   error, 7.232 mA rather than 0.034 mA, flowing whenever the ADR-0025 shorting
   plug is fitted, which is always), and `R_MODEPD` is 10 kΩ, not 100 kΩ.
   0.200 A is the next admissible declaration above BOTH counts (156 mA and
   173 mA) and a member of the standard set the bound block below declares.
   **The remaining 11.8 mA delta is OWED, not closed.**
...
**The DISSIPATION half passes, and here is the whole result.**
E-TOPO, `RAW_EXIT=0`: **PD 410 mW vs 497 mW → 82 %**. The junction temperature
that actually matters: `Tj = 75 + (0.4098 + 0.0578) × 90 = 117.1 °C`,
**7.9 °C of margin to 125 °C**, or 3.9 °C once the coincident dual-heater
excursion is carried. That margin is thin, and it is thin because the declared
load absorbs a disputed count rather than hiding it.

**THE DROPOUT HALF IS NOT CLOSED BY THIS ADR AND MUST NOT BE READ AS IF IT
WERE.** E-TOPO still prints `headroom 1355 mV vs dropout 1300 mV` off the
`vin_min: 4.754` this ADR deliberately did not touch — and the v1.7 re-gate-2
topology lens then measured, on this board, that the series resistance that
number is built from **omits the board's own copper**. `power_tree.yaml` counts
only F1 + Q_REV + eFuse = 190.5 mΩ; the routed path `J_PWR.1 → F1 → Q_REV →
U_EFUSE → U_LDO.3` carries a further **~109 mΩ** of track, and **no 5 V net has
a zone anywhere on this board**, so tracks are the entire conductor. I
reproduced the order of magnitude independently (series-sum upper bounds per
net at 35 µm / 70 °C: `5V_IN` 26.07 mΩ over 22.120 mm, `5V_FUSED` 16.23 mΩ,
`5V_RPP` 28.27 mΩ, `5V_PROTECTED` 306.63 mΩ over 260.059 mm — every one of the
lens's per-segment figures falls inside its own net's bound). **That is the
same defect class as (1) — a load-bearing sum with a term missing — on the
other half of the same rail, and it is a SEPARATE P0 that this ADR does not
resolve.**

**The declared current went DOWN, 0.30 → 0.20, in the same pass that ADDED
...
**18.6 mA**, roughly 16× less than the number that claimed to cover it. The
sensor rails it omitted are 4.6× larger than the logic it counted. Both errors
...
TYPED** (canon M-BOUND). Everything above is arithmetic over datasheet
constants; this is the one inequality the decision actually rests on, and the
whole point of this pass is that a number nobody re-derives is a number nobody
has checked.
...
id: LDO_IOUT_MAX
claim: >-
  Largest STEADY-STATE 3V3 output current U_LDO (AMS1117-3.3, SOT-223) may
  deliver while its junction stays at or below the ds1117 limit of 125 C, at the
  BRIEF's hard enclosure ambient of 75 C, with theta_JA taken at the datasheet's
  own SOT-223 package figure of 90 C/W and the regulator's own quiescent burn
  (Vin_max x Iq_max) counted alongside the pass-element dissipation.
relation: "<="
value: 0.2429
unit: A
corner: worst_case
command: /usr/bin/python3 -c "print(((125-75)/90 - 5.250*0.011)/(5.250-3.201))"
governs:
  evaluate: /usr/bin/python3 -c "I={value}; print(75 + (I*(5.250-3.201) + 5.250*0.011)*90)"
  budget: "<= 125"
  unit: C
standard_value:
  explicit: [0.10, 0.15, 0.20, 0.25]
  series_why: >-
    There is no part to buy here and saying so is the honest form of this field.
    The admissible set is the load-budget DECLARATIONS `power_tree.yaml` can
    carry for this rail, quantised to the 0.05 A step this project already uses
    (`5V_KEY_RELAY` 0.15, `5V_STOP` 0.02, `3V3_ANALOG` 0.05). The bound must
    rule on the next declaration up from the chosen one, which is 0.20 A.
chosen: 0.20
tolerance: 0.0001
tolerance_why: >-
  `value` is rounded DOWN to four decimals from an exact 0.24295049 A, so it can
  only ever understate the ceiling. 0.0001 is one unit in the last declared
  place and 429x smaller than the 0.0429 A gap to the nearest admissible
  declaration (0.20 A) the bound has to rule on.
grade: CITED
requires: [projects/smc0985-cooksense/03_src/cooksense/rules/power_tree.yaml]
```

Re-derived at the corner it declares: at the bound itself Tj = 124.99 C, at the
nearest admissible declaration (0.20 A) Tj = 117.08 C, which is also the
chosen value. **The bound sits on its own budget edge and nowhere else**, which
is exactly the property the cooksense `R_pd <= 592 Ohm` incident found missing.
