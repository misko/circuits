...
## 2026-07-31 — VIA-IN-PAD / PASTE-WINDOW LENS (v1.7, post-seal)

`2026-07-31_v1.7_via-in-pad_paste-window-lens.md` · **design_verdict DEFECTIVE ·
order_verdict BLOCKED-SOURCING**. Re-opens v1.7 layout **P2-2** and v1.2 layout
**P2-2**, both of which were dispositioned **"recorded"** without a paste-window
analysis.

| finding | measured | disposition |
|---|---|---|
| **The "276" is 275 distinct vias, and 271 of them are assembly defects** | Shipped gerbers + drill, cross-checked against `pcbnew`: **254** have the 0.150 mm hole mouth **100 %** under a printed `F_Paste` aperture, **17** are 62–95 % covered, **4** are clear. The 4 clear ones are the 1.5 mm test points `TP_5VKR/TP_3V3/TP_ESTOP/TP_USEL` — nothing is soldered to them. | **ESCALATED P2 → P1. NOT dispositionable; v1.7 needs a supersede.** The remedy is an ORDER-FORM line, not copper. |
| **The count double-counts one via and its "168 distinct pads" does not reproduce** | 276 pad-object hits on **275** vias (one `U_EFUSE` via lies in both the copper land pad 9 and a paste-aperture-only pad object). Distinct lands **273** by coordinate / **266** by (ref,pad) / **162** components — 168 matches none. | **RECORDED as a correction to the sealed text.** Sealed releases are never retro-edited; this ledger carries the correction. |
| **…and it is also an UNDERCOUNT: 5 more holes are clipped by paste with their centre outside every land** | Gerber-only sweep of all 1097 vias: 252 fully under + **24** clipped (not 19) + 5 annulus-grazed + 5 mask-open-only + 811 inert. | **RECORDED.** The centre-in-pad test is not the right test in either direction. |
| **Tented from BELOW only — the solder side is the open side** | `F_Mask` open over the hole on **275/275**; `B_Mask` open on **0/275**, nearest `B_Mask` flash **2.519 mm**. `F_Mask` has **no** 0.15/0.25 mm circular aperture — no per-via relief exists; every opening is the host land's own window. The `.kicad_pcb` declares `(tenting front yes back yes)` and is fabricated open. | **RECORDED as the mechanism.** A per-via tenting flag cannot survive the mask aperture of the pad the via sits in, and no gate compares the two. |
| **`U_EFUSE` (TPS259573DSGR) pads 2 and 5 are worse than rx2-v2's worst** | 0.600 × 0.250 mm lands with a 0.250 mm annulus: **0.0000 mm of copper rail** — the annulus is the land's whole width. rx2-v2's `VDD`/`V3` had 0.025 mm. Barrel = **3.21 ×** the entire joint at a 0.120 mm stencil. **No mask dam of any width fits.** | **This is the row that makes POFV mandatory rather than preferable.** |
| **Safety-chain nets are in the affected set** | `MCU_RELAY_ENABLE` and `STOP_REQ` (`U_OENAND.1`, `U_STOPINV.1`, 1.025 × 0.350 mm, 0.050 mm rail, 70.8 % covered); `ESTOP_OK`, `CTR_SAFE`, `FAULT_LATCH_CLEAR`, `CONTACTOR_REQ`, `RAIL_EN_A/B/RHE` among the 17 partials. | **RECORDED.** Raises consequence, not probability. |
| **Only 7 of 275 are the ordinary "via in a thermal pad" case** | Land types: exposed/thermal GND ≥4 mm² **7**; large non-GND **2**; fine-pitch ≤0.40 mm short side **22**; narrow 0.40–0.70 mm **202**; ordinary >0.70 mm **42**. By net: GND **154**, power **77**, signal **44**. **No EP is windowed** — `U_LDO.4` 97.2 %, `J_PWR.MP` 100.0 %, `D_TVS.2` 99.1 %, `CE1.2` 99.0 %, `D_REVCLAMP.2` 98.8 % coverage, one undammed aperture each. | **RECORDED.** rx2-v2's EP at least had a 3×3 window array at 66.9 %; this board has none. |
| **133 joints have a barrel bigger than the solder printed for the whole joint** | 1.600 mm board, 0.02827 mm³ per barrel, SAC305 T4 @ 50 % metal by volume: ≥100 % on **133** joints at 0.100/0.120/0.127 mm stencil (the number is set by land:aperture, not by the print). Board-wide the 275 barrels absorb **25.6 %** of all printed solder at 0.100 mm. | **RECORDED as the magnitude.** |
| **v1.2's P2-2 was withdrawn on a false premise and the population then grew 2 → 275** | The refutation reads *"Layout withdrew via-in-pad (SKILL.md records 0.25/0.15 as proven orderable)"*. Via **SIZE** being orderable is a different question from via **COVERING**, which is a separately selected option on the same form. | **RECORDED as the process failure.** The two questions must never again be answered by one sentence. |
| **`ORDER_README` §3 has no via-covering row and no order-day gate item** | Sealed v1.7 `ORDER_README.md`: `POFV` 0 hits; `resin`/`epoxy`/`via fill`/`filled via`/`plugged via`/`via covering` 0 hits. The one `via-in-pad` hit is inside the *Via tier* row (drill size). `fab_tiers.yaml` sets `via_in_pad: true` for this board's tier and **nothing reads it**. | **OWED to v1.8**, which is already required for `C265111`. Zero schedule cost. |
| **JLCPCB price for 4-layer POFV: DECLARED `catalog_absence`** | Read 2026-07-31 from JLC's own pages and the order form's own client bundle: the **Via Covering** dropdown exists, **Epoxy Filled & Capped is selectable on a 4-layer order in the standard form** (no quote needed), the **4-layer default is "Plugged"**, free upgrade is **6-layer+ only**, **expedited build is unavailable** with it, capability band **0.15–0.55 mm** (form says untented ≤0.3 / tented ≤0.5 — a published conflict). **No setup fee, rate, unit or POFV-specific MOQ is published anywhere**, including the Jan-2026 extra-charge article whose 15-case list has zero matches for via covering. | **DECLARED ABSENCE, with the verbatim question to put to JLC in §7b of the lens file.** No number may enter this repo except from a rendered quote or a JLC reply. Snippet-sourced prices are REFUSED (Q-SNIPPET). |
| **Fleet: `pluto-cal-switch` is the same shape and worse per via** | DERIVED (no sealed release; gerbers exported from a copy of `04_kicad` out of tree): **91** via-in-pad vias, **90** with the hole under or clipped by paste, `F_Mask` open 91/91. **75 are 0.300 mm drill / 0.600 mm OD — a 4× barrel.** **65 have a NEGATIVE copper rail**, including 10 on `U_MCU` (RP2040) 0.200 × 0.875 mm lands carrying `XIN`, `XOUT`, `USB_DM_MCU`, `QSPI_SCLK`, `VREG_VOUT`. | **REFERRED to the cal-switch owner as a ROUTING constraint, not an order option** — it is pre-release, and 0.300 mm sits at the order form's *"untented ≤ 0.3 mm will be filled"* boundary. |
| **No gate anywhere compares the via-covering declaration to the emitted apertures** | Three boards, three independent discoveries, three dispositions. `fab_tiers.yaml`'s `via_in_pad:` has no consumer; the `.kicad_pcb`'s `(tenting/plugging/filling/capping)` is compared to nothing; the gerbers carry the truth and are already exported at seal. | **PROPOSED, NOT APPLIED** — `skills/` changes are not this lens's to make. |
