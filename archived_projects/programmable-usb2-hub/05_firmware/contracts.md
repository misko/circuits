...
| `target/**` `host/**` | the two-sided split when the board ships MCU firmware under `target/` and the USB-host control counterpart under `host/` | both sides implement the same documented protocol; host code stays beside the firmware it controls |
